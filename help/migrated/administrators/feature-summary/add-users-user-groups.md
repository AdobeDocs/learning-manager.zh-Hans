...
<a href="https://learningmanager.adobe.com/app/learner?accountId=98632#/course/7555694">
   <button type="button">启动培训</button>
</a>
...
## 针对区分大小写的活动字段和活动字段值的工作流程

在此版本中，Learning Manager的用户属性及其值需区分大小写。 **例如**，区分大小写时，用户属性为“location”，其值“PARIS”将予以保留并以相同方式显示。 如果出现任何问题，管理员现在可以编辑属性名称和值，以更正任何有关大小写的错误。
...
1. 在&#x200B;**“添加外部注册个人资料”**&#x200B;对话框中，添加以下详细信息：
   ...
   * 在 **[!UICONTROL 高级设置]** 部分，输入以下内容：

      * **[!UICONTROL 登录要求]：** 以天为单位指定一个值。 如果学习者在上述期间没有登录，则会被删除。
...
您可以随时撤消外部合作伙伴的暂停状态并恢复正常服务。点击 **[!UICONTROL 动作]** 在页面右上角，然后选择 **[!UICONTROL 继续]**.
...
选择一个用户，然后从右上角单击 **[!UICONTROL 动作]** > **[!UICONTROL 删除用户]**.

## 更改个人资料 {#changeprofile}

要将用户移动到其他外部个人资料，请从右上角选择一个用户，单击 **[!UICONTROL 动作]** > **[!UICONTROL 更改配置文件]**. 从个人资料列表中，选择个人资料，然后单击&#x200B;**[!UICONTROL “更改”]**。

## 分配角色 {#Assignarole-1}

选择一个用户，然后从右上角单击 **[!UICONTROL 动作]** > **[!UICONTROL 分配角色]** > **Make`<role>`**. 用户将获得新角色。

## 删除角色 {#Removearole-1}

选择一个用户，然后从右上角单击 **[!UICONTROL 动作]** > **[!UICONTROL 删除角色]** > **移除`<role>`**. 所选角色将从分配给用户的角色列表中删除。
...
点击 **[!UICONTROL 数据源]** 左侧窗格中的 **[!UICONTROL 高级]** 部分。
...
从中选择数据源类型 **[!UICONTROL 来源]** 下拉列表，选择更新频率，然后单击 **[!UICONTROL 立即同步]** 如果需要立即同步或单击 **[!UICONTROL 保存].** 内部用户的数据源类型有SFDC、FTP等。
...
点击 **[!UICONTROL 活动字段]** 可在“用户”页面中使用。 学习者只能从自定义值提供的值中进行选择。
...
如果选项 **在学习者登录时仅显示未填写的字段** 已启用，则用户在登录时只会看到空白字段。
...
Learning Manager 27.7引入了两个新选项： **[!UICONTROL 可报告]** 和 **[!UICONTROL 可导出]**，用于活动字段。
...
将您选择的任何字段添加为用户信息的一部分，然后单击 **[!UICONTROL 保存]**. 添加字段后，您还可以在 **[!UICONTROL 编辑用户]** 对话。
...
在此空间中，您可以查看管理员使用批量导入功能添加的，用户 CSV 导入历史记录。您也可以单击 **[!UICONTROL 添加]** ，以使用CSV上传功能添加用户。
...
1. 在“设置”选项卡中，将新字段标记为多值字段。
   ...
   *标记为多值*
   ...
   还有一个复选框， **[!UICONTROL 学习者可配置]**，禁用后，学习者将无法在“配置文件”页面中看到该字段。
...
1. 选择用户后，单击 **[!UICONTROL 动作]** ，然后单击 **[!UICONTROL 注册]**.
...
在“外部用户”列表中，选择要删除的用户。 在右上角，单击 **[!UICONTROL 动作]** > **[!UICONTROL 暂停]**.
...
1. 点击 **[!UICONTROL 用户]** > **[!UICONTROL 活动字段]**.

1. 点击 **[!UICONTROL 设置]** 并启用该选项 **[!UICONTROL 在学习者登录时仅显示未填写的字段]**.
