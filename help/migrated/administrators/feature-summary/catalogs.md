...
[![按钮](assets/launch-training-button.png)](https://content.adobelearningmanageracademy.com/app/learner?accountId=98632#/course/8318922)
...
1. 以管理员身份，单击左窗格中的“目录”。 在该页面上，有一个您创建 Adobe Learning Manager 帐户时自动创建的默认目录。 该目录中的所有学习对象均默认与所有学习者共享。
...
1. 在新目录的页面上，单击左窗格中的&#x200B;**[!UICONTROL “内容”]**。
   ...
   已添加的学习计划会显示在&#x200B;**[!UICONTROL “学习计划”]**&#x200B;选项卡中。
...
或者，您可以选择目录旁边的复选框，单击右上角的“操作”，然后单击&#x200B;**[!UICONTROL “禁用”]**。

您还可禁用列表中的多个目录。 选择目录，然后从“操作”菜单中选择&#x200B;**[!UICONTROL “禁用”]**&#x200B;选项。
...
[![按钮](assets/launch-training-button.png)](https://content.adobelearningmanageracademy.com/app/learner?accountId=98632#/course/8318924)
...
输入与您的 Adobe Learning Manager 帐户相关联的子域。 单击&#x200B;**[!UICONTROL 如何获取子域？]** 了解子域的使用情况。 您也可单击管理员主页左窗格中的&#x200B;**[!UICONTROL “品牌推广”]**&#x200B;以查看与您的公司帐户相关联的子域。
...
您可以通过在&#x200B;**[!UICONTROL 到期]**&#x200B;字段中设置到期日期来设置共享目录的有效性。 在共享目录的到期日期之后，将不再允许注册课程。 现有学习者仍可访问内容。 学习对象已弃用，但徽章和技能仍处于活动状态。 在这种状态下，共享目录接收者无法取消弃用已弃用的学习对象。

添加所有必填信息后，单击&#x200B;**[!UICONTROL “保存”]**。 单击页面右上角的&#x200B;**[!UICONTROL “操作”]**&#x200B;下拉菜单，然后单击&#x200B;**[!UICONTROL “共享”]**&#x200B;以与外部帐户共享目录。
...
取消共享目录即禁用应用程序中的外部帐户。 要取消共享目录，请单击页面右上角的&#x200B;**[!UICONTROL “操作”]**，然后单击&#x200B;**[!UICONTROL “取消共享”]**。 您可以随时取消共享目录。 目录取消共享时，该共享目录中的所有学习对象均会变为弃用状态。 禁用的目录在接收者的应用程序中显示为已弃用。

只有在接收者尚未接受请求时才能从共享列表中删除外部帐户。

**删除目录**

要删除共享目录，请单击&#x200B;**[!UICONTROL “操作”]**&#x200B;下拉菜单，然后单击&#x200B;**[!UICONTROL “删除”]**。

* 您无法与外部帐户共享默认目录。
* 您无法删除您已进行外部共享的目录。
* 您无法删除其他帐户共享给您的目录。

## 限制对已获取的课程/学习路径所做的更改 {#restrictchangesonacquiredcourseslp}

在使用帐户共享的组织中，学习内容通常在主帐户中创作和维护，然后通过共享目录与配对帐户共享。 虽然此模型有助于在多个帐户之间维护一致的内容，但也会导致在接收帐户中进行意外修改。

这有助于保护共享学习对象，防止接收帐户的作者进行可能会破坏源帐户与共享内容之间关系的更改。 这允许内容所有者维护集中管理，同时确保源帐户的更新继续流向接收帐户。
...
* 防止可能破坏与源帐户同步的意外修改。
* 跨多个帐户减少内容管理开销。
* 确保在源帐户中进行的更新可继续传播到接收帐户。
* 在作者尝试修改共享内容时，向其提供明确警告。

将目录从源帐户共享到接收帐户后，课程、学习路径和认证等学习对象将在接收帐户中变为可用。

如果接收帐户启用“限制对获取的课程/学习路径的更改”，则Adobe Learning Manager会限制对从共享目录发起的继承内容的编辑。 作者仍可使用和使用共享内容，但他们无法进行可能会切断与源帐户关系的更改。
...
2. 选择&#x200B;**目录**。
3. 选择共享目录。
4. 在共享目录的&#x200B;**概述**&#x200B;部分中，选择切换&#x200B;**限制对已获取的课程/学习路径进行更改**。 启用后，已获取目录的“课程/学习路径”部分的更改会受到限制，以允许更新从主帐户流向此帐户。
...
当作者打开通过共享目录获取的课程时，系统会显示一条信息性消息，指示：
...
* 作者可能会看到一条警告，指示课程是从源帐户获取的。
* 此警告说明修改内容可能会中断与源帐户的关系。
* 根据设置，系统限制或防止对受影响的内容进行更改。

这有助于作者在进行更改之前了解其操作的影响。

**哪些更改受到限制？**

启用编辑限制后，对继承课程内容的修改会受到限制，因为这些组件由源帐户维护。 这些限制旨在保持帐户之间的同步。
...
目标是防止更改可能导致接收帐户的副本与源版本存在差异。

**仍允许哪些更改？**

某些设置将继续在接收帐户内独立管理，并保持可编辑状态。 如下所示：
...
企业学习团队在源帐户中维护合规性课程，并与多个区域帐户共享。

区域帐户中的作者可直接编辑已获取的课程。 因此，与原始课程的连接可能会中断，从而阻止来自源帐户的未来更新正确流动。

#### 限制后

接收帐户可启用&#x200B;**限制对已获取的课程/学习路径的更改**。

区域作者访问共享课程时，可以查看和分配内容，但无法修改保护字段。 源帐户所做的更新将继续传递给接收帐户，而不会发生中断。
...
* 共享目录中的学习对象会显示在作者和管理员应用程序中；但模块会隐藏起来，因此不会列出。
* 共享学习对象可用于接收者帐户中的所有工作流程。 例如，将共享课程添加到学习对象，如认证（永久和循环）、学习路径和目录等；但是，无法再次共享这些学习对象（因为它们已经拥有来自其他帐户的共享学习对象）。
...
要添加课程、认证和学习计划到目录，请参阅[步骤](catalogs.md#q1)以将这些内容包含在目录中。
