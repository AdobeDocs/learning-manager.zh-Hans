...
作者或管理员可以输入位置名称，以查看显示的相关结果。 作者或管理员随后可以从显示的结果中选择一个位置。 如果预先输入的结果中未显示任何位置，用户仍然可以添加新的教室位置名称。 请注意，使用会话创建工作流程创建的位置名称不会添加到由管理员创建的位置库中。
...
因此，在管理员启用&#x200B;**[!UICONTROL “讲师管理”]**&#x200B;选项之后，只能将具有讲师角色的用户添加为讲师。
...
在&#x200B;**[!UICONTROL “课程实例”]**&#x200B;页面中，管理员可以取消一个或多个会话。 管理员取消会话后，系统会清除除名额限制之外的所有会话详细信息。
...
请注意，即使会话已取消，已注册培训实例的学习者仍会保持原有注册状态。 其注册状态（包括已确认、轮候和等待经理审批）不会发生变化。 此功能在管理员日后设置和重新安排已取消的会话时非常有用。
