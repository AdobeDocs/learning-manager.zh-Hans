...
您可以使用相同的CSV文件格式进行注册、出席和完成。 只需在“电子邮件”列下输入学习者电子邮件ID，并根据操作（例如bulk_enrollment.csv、bulk_authence.csv或bulk_completion.csv）使用名称保存文件。 仅支持CSV格式。 不支持UTF-8格式。 在[此处](assets/Sample-Bulk-Action-CSV.csv)下载示例csv。
