...
1. 输入实例的属性：
   ...
   * 在 **[!UICONTROL 实例名称]** 字段中，输入要与该课程关联的实例名称。 确保对实例使用唯一的名称。
   ...
   * **[!UICONTROL 时区]：** 搜索，然后选择 **[!UICONTROL 时区]** 从下拉列表中。
...
1. 在 **[!UICONTROL 实例]** 页面，选择任意实例上的选项，然后单击 **[!UICONTROL 电子邮件注册学习者]**.
...
如要允许学习者重置模块并再次继续学习，请单击 [!UICONTROL **重置**]. 从弹出式对话框中，单击“是”以确认重置。 已完成的模块不能重置。 只有未通过或未完成的模块才能重置。

您也可以将学习者列表导出到 Excel 表格中。 要导出学习者列表，请单击 [!UICONTROL **动作**] > [!UICONTROL **导出**].

>[!NOTE]
>
>如果一门课程存在多个实例，则在各标签下单独提供Excel格式的学习者列表。 学习者列表包括学习者姓名、状态和选择条件。 学习者状态可以是 **未开始**，或者 **进行中**，或者 **已完成**.
...
如果课程以多种语言提供，则测试报告也会以不同的语言显示数据。 到目前为止，管理员生成的报告会依序逐一显示回应，而不考虑参与测试时使用的语言为何。 **例如**，如果用户以荷兰语参加测试，管理员每次只能查看用户以荷兰语完成测试的报告。 将英语作为界面语言的管理员无法同时查看所有用户的报告，与参与的区域无关。
...
作为管理员，您可以查看所有可用课程的列表。   点击 **[!UICONTROL 课程]** 在左侧窗格中查看包含搜索和筛选选项的课程列表。 您还可以在课程缩略图中查看每门课程的课程效果百分比。

>[!NOTE]
>
>在学习者使用完课程后或者在任何特定的课程发布后您想要搁置该课程时，可以弃用课程。 您仅可弃用处于已发布状态的课程。单击以下链接可查看所有已弃用的课程 **[!UICONTROL 已弃用]** 选项卡。
...
管理员可以在以下位置设置默认徽章、游戏设置和提醒： **[!UICONTROL 默认实例]** 页面。 要修改默认实例设置，请选择 **[!UICONTROL 默认实例]** > **[!UICONTROL 编辑]**.

* **[!UICONTROL 徽章]**：从下拉菜单中选择默认徽章。
* **[!UICONTROL 游戏]**：配置游戏设置，包括完成点数、提早完成点数和及时完成点数。 管理员可以选择帐户级别设置或自定义此实例的游戏点数。
* **[!UICONTROL L1反应反馈]**：在课程完成时为学习者反馈启用预定义问题，并提供将问题设为必答的选项。
***[!UICONTROL L3行为更改反馈]**：在课程完成后，为学习者的经理启用反馈问题。
