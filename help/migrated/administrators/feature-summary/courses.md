...
2. 在课程列表中，选择所需课程，然后单击&#x200B;**[!UICONTROL “查看课程”]**。

   ![](assets/view-course.png)

   *查看课程*

3. 要创建实例，请单击左侧窗格中的&#x200B;**[!UICONTROL “实例”]**。 默认情况下，每个课程都有一个实例。 您可以修改默认实例或添加实例。 您无法删除此课程实例。
4. 要创建示例，单击课程信息右上角的&#x200B;**[!UICONTROL “添加新实例”]**。 此时会显示课程的新实例。
5. 输入实例的属性：

   * 在&#x200B;**[!UICONTROL 实例名称]**&#x200B;字段中，输入要与该课程关联的实例的名称。 确保对实例使用唯一的名称。
   * **开始日期：**&#x200B;这是实例的开始日期。 您可以在两个不同的选定日期在此开始日期之前向学习者发送两封提醒电子邮件。 开始日期必须早于完成日期。 选择开始日期后，该日期将在实例的&#x200B;**通知警报**&#x200B;部分的日历中以蓝色突出显示。
...
## 批量管理学习者注册、出席和完成情况 {#bulk-enrollment}
...
管理员可以通过上传CSV文件同时注册最多100,000个用户，而不是一次添加一个学习者。 该文件必须包含一个标记为&#x200B;**userEmail**&#x200B;的列，其中包含要注册的学习者的电子邮件地址。

要使用CSV批量注册学习者，请执行以下操作：

1. 以管理员身份登录。
2. 从&#x200B;**[!UICONTROL 课程]**&#x200B;部分中选择课程。
3. 在&#x200B;**[!UICONTROL 课程概述]**&#x200B;页面中选择&#x200B;**[!UICONTROL 学习者]**。
4. 选择&#x200B;**[!UICONTROL 注册]**，然后选择&#x200B;**[!UICONTROL 上传CSV]**。\
   ![](assets/upload-a-csv-learners.png)
   _使用CSV上传功能注册学习者_
5. 上传CSV并选择&#x200B;**[!UICONTROL 继续]**。

该CSV文件包含标记为“用户电子邮件”的列。 在此列中输入您的用户的电子邮件地址。

### 批量标记课程完成

管理员可以通过上传包含学习者电子邮件地址的CSV文件，快速标记多个学习者的课程完成情况。 与单独更新每个学习者相比，这节省了时间。 CSV的用户电子邮件列会显示要更新的学习者。 一次上传最多可将10,000名学习者标记为完成。

标记批量完成：

1. 从&#x200B;**[!UICONTROL 课程]**&#x200B;部分中选择课程。
2. 在&#x200B;**[!UICONTROL 课程概述]**&#x200B;页面中选择&#x200B;**[!UICONTROL 学习者]**。
3. 选择&#x200B;**[!UICONTROL 操作]**，然后选择&#x200B;**[!UICONTROL 标记完成]**。
4. 选择&#x200B;**[!UICONTROL 批量]**。
5. 上传一个CSV文件，其中包含列出了已完成课程的学习者的“用户电子邮件”列。
...
管理员可以使用批量出勤功能一次标记多个学习者的出勤情况。 管理员可以上传包含学习者电子邮件地址的CSV文件，而不是单独更新每个学习者的出勤情况。 CSV中的userEmail列可标识要记录的学习者出勤情况。 此流程可在单个上传中处理多达10,000名学习者，让出勤标记更加快速高效。

标记批量出席情况：

1. 从&#x200B;**[!UICONTROL 课程]**&#x200B;部分中选择课程。
2. 在&#x200B;**[!UICONTROL 课程概述]**&#x200B;页面中选择&#x200B;**[!UICONTROL 出席情况与得分]**。
3. 选择“**[!UICONTROL 操作]**”，然后选择“**[!UICONTROL 标记已批量参加]**”。
4. 上传一个CSV文件，该文件应包含userEmail列，并提供您要更新其出勤情况的学习者的电子邮件地址。
...
>[!NOTE]
>
>您可以使用CSV批量标记最多10,000个用户的出席情况。

### 常见CSV上传错误

* CSV中的学习者电子邮件不存在Adobe Learning Manager用户目录。
* 文件格式不正确。
* 该文件包含额外列或无效数据。
...
您可以在行级别下载并查看列出失败用户错误的CSV文件，以便于识别。
...
>[!NOTE]
>
>仅在学习计划实例中启用这些设置是不够的。 而且必须还要在“学习计划”每门课程的“课程实例”级别中启用这些设置。

在“实例默认值”页面，如果启用&#x200B;**[!UICONTROL “将所有问题全部设为必答题”]**，则此后创建的所有新实例都将沿用这些设置。
...
在当前版本的 Adobe Learning Manager 中，管理员可就“学习计划”囊括的所有课程发送 L1 反馈。 管理员必须确保为课程实例级别的所有课程启用 L1 反馈。

1. 要为每个课程启用L1反馈，请在管理员应用程序中单击&#x200B;**[!UICONTROL 学习计划]** > **[!UICONTROL 查看学习计划]**。

1. 单击&#x200B;**[!UICONTROL 实例]** > **[!UICONTROL 已启用L1反馈]**。

1. 启用&#x200B;**[!UICONTROL “为每门课程启用”]**&#x200B;选项。

   ![](assets/enable-l1-feedbackforcourse.png)

   *启用课程反馈*

   若只在“学习计划”级别启用此切换功能，系统将不会为此计划所含的课程触发 L1 反馈。 要启用 L1 反馈，请前往“学习计划”中的每门课程，并启用 L1 反馈切换功能。

   ![](assets/l1-reaction-feedback.png)

   *为每门课程启用L1反馈*

   若为所有课程启用 L1 反馈，但在“学习计划”实例中禁用此反馈，系统将不会为课程触发 L1 反馈。

### 特定于语言的测试报告

测试报告有助于评估学习者在完成学习计划或课程后的表现。

Adobe Learning Manager 目前支持 13 种界面语言和 32 种内容语言的学习。 虽然对学习者而言，此选项操作轻松，并且能够方便地支持我们全球的学习者，但是管理员要获取不同区域的测试报告并不容易。
...
管理员将能在帐户级别打开启用设置，以便为新创建的课程和“学习计划”启用 L1 反馈。 但是，打开此设置不会影响现有的课程和“学习计划”
...
1. 以管理员身份登录后，依次单击左侧窗格中的&#x200B;**[!UICONTROL “设置”]**&#x200B;和&#x200B;**[!UICONTROL “反馈”]**。\
   此时会显示反馈设置页面，其中包含两个选项卡：**[!UICONTROL L1反馈]**&#x200B;和&#x200B;**[!UICONTROL L3反馈]**。\
   **[!UICONTROL L1反馈]**&#x200B;选项卡包含教室和自学课程的默认&#x200B;**[!UICONTROL L1反馈]**&#x200B;问卷列表以及提醒设置。 在&#x200B;**[!UICONTROL “L3 反馈”]**&#x200B;选项卡中，可查看 L3 反馈默认语句和提醒设置。
...
管理员可查看所有可用课程的列表。   单击左侧窗格中的&#x200B;**[!UICONTROL 课程]**&#x200B;以查看包含搜索和筛选选项的课程列表。 您还可以在课程缩略图中查看每门课程的课程效果百分比。
...
管理员在将任何学习者的课程、学习路径或认证标记为完成时，可以发表评论。 这些注释有助于实现合规性和审核目的。 管理员可以一次轻松为一个或多个学习者添加注释。
...
管理员可以单击&#x200B;**[!UICONTROL “学习者预览”]**&#x200B;选项，同时查看课程模块。
...
1. 使用“搜索”字段。 单击右上角显示的搜索图标。 此时会显示搜索字段。 输入课程名称或课程相关的任何关键词来查找您的课程/学习计划。 您还可以使用 Captivate、C、Java 和 HTML 等预定义标签进行搜索。 标签可在搜索字段中进行搜索，也就是说标签会在您输入时显示在搜索字段中。
1. 使用过滤器过滤课程/学习计划列表。 您可以按状态过滤课程，例如“全部”、“已发布”、“草稿”和“已弃用”。 在管理员模式下，不会显示草稿过滤器。

您可以单击“能力”并进行选择，然后根据能力进行搜索。 管理员可以通过四种方式对课程进行排序，以更好地找到您所需的课程。 单击“排序方式”并选择字母升序、字母降序、课程更新日期或课程效果。
...
在&#x200B;**[!UICONTROL “注册学习者”]**&#x200B;对话框中，展开&#x200B;**[!UICONTROL “高级选项”]**&#x200B;选项。 在&#x200B;**[!UICONTROL “其他注释”]**&#x200B;字段中，输入所需注释。
...
**已删除** — 课程从Adobe Learning Manager应用程序中完全移除后，即显示为删除状态。 课程只有处于草稿状态时，作者才能将其删除。 您还可以删除处于弃用状态的课程。
