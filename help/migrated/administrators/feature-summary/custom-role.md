...
4. **[!UICONTROL 帐户权限]**：这些权限授予角色所有者访问特定系统配置方面的权限，这些权限作用于整个帐户。 选择访问权限。 用户可完全掌控所分配的权限。
   ...
   *设置作用域*
   ...
   ### 电子邮件模板布局需要具有电子邮件模板帐户权限

   若要查看正确呈现布局的课程级电子邮件模板，自定义角色需要&#x200B;**和**&#x200B;以下各项：

   * 使用“功能权限 — 学习对象”可完全访问&#x200B;**课程**
   * 以帐户权限访问&#x200B;**电子邮件模板**
...
HR运营团队负责在员工离开组织时清理帐户。 他们需要定期清除已删除的用户，但无权访问课程内容、学习者数据或系统设置。 授予高级完全控制访问权限（仅限用户管理）可为HR团队提供清理和导入用户所需的具体访问权限，而无需公开任何其他管理功能。
...
>[!NOTE]
>
>您最多可以为每个用户分配50个角色，并为每个角色添加最多3500个用户。
...
如果CSV文件包含的数据不正确或超出限制（每个用户50个角色，每个角色3500个用户），则会出现一条消息，显示错误。
