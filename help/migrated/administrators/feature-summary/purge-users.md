...
1. 打开&#x200B;**[!UICONTROL 从“操作”]**&#x200B;下拉菜单，然后选择&#x200B;**[!UICONTROL “清除用户”]**。
...
>[!NOTE]
>
>将被删除的用户添加回系统后，之前的角色（例如管理员、经理、作者、讲师等） 将不会保留。他们将被添加“学习者”角色。
...
<table>
 <tbody>
  ...
  <tr>
   <td><strong>使用 Adobe Learning Manager 用户界面清除 — 非企业版</strong></td>
   <td> </td>
  </tr>
  <tr>
  ...
  <tr>
   <td><strong>清除其他用户 — 企业版（非内部或外部 Adobe Learning Manager 用户的个人）</strong></td>
   <td> </td>
  </tr>
  <tr>
...
### 自动清除已删除的用户{#auto-purge}

自动清除已删除用户功能可清除已在ALM中删除的用户的数据。 清理在可配置的保留期之后进行，侧重于批量操作，以便在不影响性能的情况下高效地处理大型客户帐户。

批量删除流程每批最多可处理10,000个用户。 该功能被定位为面向大批量删除的注重可靠性的后台服务。

作为管理员，您可以指定删除已清除用户的持续时间。 有关详细信息，请查看[管理设置](/help/migrated/administrators/feature-summary/settings.md)。

#### 它的功能：

* 在帐户级别为已删除用户提供可配置的自动清除功能
* 确保用户在符合清除标准后，在24小时内被清除
* 支持每天批量删除多达10,000个用户**而不会降低系统性能
* 在这些操作运行时维护总体系统响应和数据库运行状况
* 强制实施自动化数据保留管理以履行GDPR义务

#### 但事实并非如此：

* 批量删除流程将仅作为计划的cron作业运行（而不是根据请求按需）

### 启用自动清除选项

1. 以管理员身份登录Adobe Learning Manager。
2. 导航至&#x200B;**配置**&#x200B;部分> **设置** > **基础知识**&#x200B;部分> **常规**。
3. 向下滚动页面至&#x200B;**自动清除已删除的用户**。
   ![](assets/auto-purge1.png)
   *自动清除选项*
   >[!NOTE]
   >
   >如果未启用&#x200B;**自动清除已删除的用户**，则该选项部分的底部将显示一条消息：**未配置**。
4. 选择&#x200B;**“编辑”**。
5. 选中&#x200B;**启用复选框**。
6. 输入清除生效的持续时间。
   >[!NOTE]
   >
   >最小值应为一年。 也可以按1递增。 但是，不能输入1.5年或2.5年之类的值。 如果您需要自定义值作为持续时间，请联系我们的客户支持。
7. 选择&#x200B;**“保存”**。 ALM会显示详细的确认消息。
