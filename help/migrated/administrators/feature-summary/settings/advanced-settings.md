...
只有管理员才能创建、编辑或删除任何级别的文件夹。 作者和自定义用户与层次结构交互，但不能对其进行修改。
...
内容库将显示每个内容文件的完整文件夹路径。 例如，**培训方案** > **入门** > **SCORM资源**。 此路径显示内容的完整位置。
...
当您选择文件夹时，例如，在过滤或删除时，选择将按如下所示在层次结构中级联：

* 选择&#x200B;**1级文件夹**&#x200B;会自动选择其下的所有2级和3级文件夹。

* 选择&#x200B;**2级文件夹**&#x200B;会自动选择其下的所有3级文件夹。 不会选择同一“1级”文件夹下的其他“2级”文件夹。

* 选择&#x200B;**3级文件夹**&#x200B;只会选择该文件夹。 未选择其他文件夹。

>[!NOTE]
>
>选择子文件夹而不选择其父文件夹时，父文件夹不显示部分或混合选择指示符。 这是有意为之。 因为父文件夹本身可以包含内容，而不只是子文件夹。 选择父文件夹意味着“包含此文件夹中的所有内容及其下方的所有内容”。 部分指示符会建议部分包含父文件夹自己的内容，这将产生误导。 如果只想按特定子文件夹进行筛选，请直接选择该子文件夹。 如果希望在父文件夹及其子文件夹中包含所有内容，请选择父文件夹。
...
区域作者团队仅获准访问其1级或2级分支。 他们只能查找、更新和重复使用与其管辖范围相关的资源，而不会看到或不小心修改了其他区域的内容。
...
* 您可以在任何级别删除空文件夹。
* 如果文件夹包含未链接到任何其他文件夹的内容，则无法删除该文件夹。 首先将该内容移动到另一个文件夹。
* 删除父文件夹会删除其所有子文件夹。 选择父文件夹将自动选择其所有子文件夹。
...
>[!CAUTION]
>
>删除文件夹是永久删除。 在确认之前，请确保文件夹中的所有内容都已移至其他位置。
...
5. **删除文件夹之前移动内容。** 如果文件夹包含未链接到任何其他位置的内容，则会阻止删除。 养成在删除前查看文件夹内容的习惯。
