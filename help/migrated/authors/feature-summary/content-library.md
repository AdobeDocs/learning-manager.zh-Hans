 ...
 <tbody>
  ...
  <tr>
   ...
   <td>
    ...
    <p><b>重要信息：学习者助理和基于SCORM的评估</b></p>
    <p>如果您的帐户启用了“学习者助手”，并且您将SCORM/xAPI包上传到配置为“学习者助手”内容源的目录中，请注意以下事项：</p>
    <ul>
      <li>学习者可以使用学习者助理询问有关这些包中内容的问题。</li>
      <li>助理可以生成解释或摘要，有效地显示对SCORM课程中嵌入的测验问题的答案（例如，通过重述反馈或解释“正确”选项）。</li>
   </ul>
   <p>对于高风险或委托进行的评估（如合规性或认证考试），Adobe建议：</p>
   <ul>
      <li>在未配置为“学习者助理”源的目录中保留大量评估内容的SCORM内容，或者</li>
      <li>设计评估，以便简单地改变学习材料不会轻易地公开正确的答案。</li>
   </ul>
   <p>预期会出现此行为：Adobe Learning Manager不会阻止学习者助理解释明确选择作为AI源的目录中的内容。</p>
    </td>
   <td>
    <p>zip</p></td>
  </tr>
...
## 预览课程内容 {#previewcontentlibrary}

将课程内容添加到内容库后，您可以预览该内容。 要预览，请选择相应课程行末的眼睛图标。
...
1. Adobe Learning Manager支持为Adobe Learning Manager中上传的视频内容提供字幕。 现在，作者可以将包含字幕的文件连同视频文件一起上传。
   ...
   字幕之后会显示在流体播放器中。 字幕还应符合 [WCAG2.0 标准](https://www.w3.org/TR/WCAG20/)。
...
* 全球培训：提供多语言字幕视频内容，以帮助国际学习者。
* 辅助功能合规性：为听力受损用户提供其首选语言的字幕。
* 更快的本地化：通过自动生成和平移VTT文件，减少手动工作并加快内容发布。
* 一致体验：确保所有学习者都收到相同的信息，无论使用何种语言。

#### 主要功能

* 自动生成VTT：上传视频或音频文件，然后自动生成原始语言的VTT字幕。
* 多语言翻译：将字幕平移为10种支持的非英语语言中的任意一种。
* 应用程序内审阅和编辑：在发布之前审阅、编辑和下载VTT文件。
* 通知：在VTT生成和转换完成后接收应用程序内通知。
* 顺利发布：Publish为学习者最终提供了字幕，供其使用所选语言进行访问。

#### 上传内容并生成VTT

1. 转到内容库并选择[!UICONTROL 添加内容]。
2. 上传MP3或MP4文件。
3. 在上传对话框中，选择&#x200B;**生成翻译**&#x200B;选项。
4. 选择原始内容语言（默认为文件的语言）。
5. 选择其他翻译目标语言（最多支持10种）。
6. 选择[!UICONTROL “保存”]。 系统开始生成和平移VTT文件。
...
* **支持的语言：** Adobe Learning Manager支持10种非英语语言（法语、德语、西班牙语、意大利语、葡萄牙语、荷兰语、瑞典语、印度尼西亚语、挪威博克马尔语和丹麦语）。
* **通知：**&#x200B;当VTT生成和翻译完成时，作者会收到通知。
* 编辑灵活性：可以在应用程序内编辑字幕，也可以离线编辑并重新上传。
...
例如，代理机构希望保持对内容的完全控制权，而未注意到内容的人员必须有权访问所有内容。 同时，机构中的内容创作者只能访问自己的内容，在某些情况下，还必须访问他人的内容。

我们将包含现有内容（即在配置内容文件夹之前上传的内容）的内容库定义为&#x200B;**公共文件夹**。 此文件夹无法弃用或删除。 公共文件夹所含的内容可供各类作者访问。 内容文件夹经配置后，标准作者和自定义作者应在上传新内容时选择用于放置内容的文件夹。
...
上传新内容时，您可以选择在上传过程中要将其存储在哪个文件夹中。

1. 以作者身份登录并导航到&#x200B;**管理** > **内容库**，或从主页选择&#x200B;**创建内容**。
...
在&#x200B;**内容库**&#x200B;中，每个内容文件都显示其完整文件夹路径，例如，**销售培训** / **自学模块** / **PDF资源**。 该路径精确地告诉您文件在层次结构中的位置。

* 如果文件存在于多个文件夹中，则所有路径都以逗号分隔。

* 如果路径很长，则会从开头以省略号(...)截断它，并且始终显示最深的文件夹名称。

* 已发布、已弃用和草稿的内容都会显示在内容库中，并显示其文件夹路径。

![](assets/cl1.png)

## 按文件夹过滤内容

内容库中的&#x200B;**文件夹**&#x200B;筛选器可让您缩小列表范围，仅显示存储在特定文件夹中的内容。

1. 在&#x200B;**内容库**&#x200B;中，选择列标题中的&#x200B;**文件夹**&#x200B;下拉菜单。
   ![](assets/cl5.png)
   下拉菜单显示公用文件夹和专用文件夹列表。 无法同时选择公用和专用文件夹。 选取其中一个可取消选择另一个。

2. 要按特定专用文件夹进行筛选，请执行以下操作：
   * 选择&#x200B;**1级文件夹**&#x200B;会自动选择其下的所有2级和3级子文件夹。 这些子文件夹显示为已选中，但无法单独取消选择。 此过滤器返回“级别1”文件夹及其子文件夹中的所有内容。
   * 选择&#x200B;**2级文件夹**&#x200B;会自动选择其下的所有3级子文件夹。
     ![](assets/cl6.png)
...
移动内容会将其从当前文件夹中删除，并放置在目标文件夹中。 您可以在对话框中选择多个专用文件夹来移动内容。 但是，不能同时选择专用文件夹和公用文件夹。 可以选择多个专用文件夹或单个公用文件夹。

1. 在&#x200B;**内容库**&#x200B;中，选中要移动的每个内容文件旁边的复选框。
2. 选择“**操作**”>“**整理内容**”>“**将内容移动到文件夹**”。 将打开&#x200B;**将内容移动到文件夹**&#x200B;面板。
3. 选择要将内容移动到的目标文件夹。
4. 选择&#x200B;**移动内容**。 内容库将更新以显示新的文件夹路径。

>[!IMPORTANT]
>
>您可以将内容从公用文件夹移动到专用文件夹，反之亦然。 但是，不能在同一操作中将内容移动到公用和专用文件夹中。 如果选择公用文件夹，则无法选择任何专用文件夹，反之亦然。
...
复制操作会创建指向其他文件夹中的内容的链接，而不将其从当前位置删除。 不会复制内容文件本身。

1. 在&#x200B;**内容库**&#x200B;中，选中每个要复制的内容文件旁边的复选框。
2. 选择&#x200B;**操作** > **整理内容** > **将内容复制到文件夹**。 将打开&#x200B;**将内容复制到文件夹**&#x200B;面板，显示文件夹层次结构。
3. 选择要在其中显示内容的一个或多个目标文件夹。
4. 选择&#x200B;**复制内容**。 内容库将进行更新，以显示所复制内容的新文件夹路径。

>[!NOTE]
>
>不能将内容从专用文件夹复制到公用文件夹，也不能从公用文件夹复制到专用文件夹。
...
如果指定文件夹&#x200B;**也与其他文件夹关联，则只能从指定文件夹**&#x200B;取消链接内容。 如果要取消链接的内容仅与一个文件夹关联，则建议改用“移动”操作。
...
向课程添加自学模块时，您可以浏览文件夹层次结构以查找正确的内容文件，而不是按名称搜索。

1. 以作者身份登录，然后导航至左侧导航栏中的&#x200B;**课程**。

2. 选择&#x200B;**添加**&#x200B;以创建新课程或在编辑模式下打开现有课程。

3. 导航到&#x200B;**模块**&#x200B;部分，然后选择&#x200B;**内容** > **添加模块**。

4. 在&#x200B;**选择模块类型**&#x200B;对话框中，选择&#x200B;**自学**&#x200B;作为模块类型。 将打开&#x200B;**自学模块**&#x200B;对话框。

5. 选择&#x200B;**浏览文件夹**&#x200B;以打开文件夹层次结构。

6. 在文件夹级别中导航以找到要添加模块。 选择内容文件。

7. 选择&#x200B;**添加**。 模块即会添加到课程中。

8. 完成所有剩余的课程字段，然后选择&#x200B;**保存**。
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>
    ...
    <p><b>注意：</b>只能编辑 Captivate、Presenter Video Express 或 Presenter 中的 HTML 内容。</p></td>
  </tr>
 </tbody>
</table>
...
内容唯一ID和到期日期适用于内容组的所有语言版本，从而确保无论使用哪种语言，所有用户都能获得一致的体验。 作者可以使用Content unique ID快速搜索和查找特定内容，从而更轻松地管理和更新培训材料。
...
在Adobe Learning Manager中，内容库还可以维护您上传内容的版本。 如果对内容（例如 PowerPoint 演示文稿）进行了更改，并重新上传更改后的演示文稿，则库中的版本号会递增 1。 这样有助于对内容的更改情况进行跟踪。 内容的新版本可同时应用于所有学习对象，您也可为每门课程应用独立的更新。
+++
