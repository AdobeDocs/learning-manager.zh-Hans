...
1. 选择 **[!UICONTROL 内容库]** 登录后位于左侧窗格中 **作者** 并选择 **[!UICONTROL 添加]** > **[!UICONTROL 内容]**.

>[!NOTE]
>
>选择 **[!UICONTROL 添加]** 将显示两个选项。 **[!UICONTROL 内容]** 和 **[!UICONTROL 测验]**：请参阅 [创建测验](content-library.md#createaquiz) 了解更多详细信息。


或者，您也可以选择 **[!UICONTROL 创建内容]** 从 **[!UICONTROL 快速入门]** 页面。

1. 在 **[!UICONTROL 名称]** 字段中，请键入要上传的内容名称。
...
1. 在 **[!UICONTROL 持续时间]** 字段，您可以选择键入学习者将花在此模块中的预计时间。 持续时间的单位为分钟。

   学习者所花费的学习时间将根据所指定的持续时间进行计算（前提是学习者已将课程标记为完成）。如果学习者使用播放器中的内容，则在播放器中所花费的时间将计入所花费的学习时间。 如果内容的实际使用时间小于指定的持续时间，则不会有任何操作，因为播放器始终遵循并显示相应内容的时间。

1. 在 **[!UICONTROL 标记]** 字段中，输入已上传内容的标记，以使可发现您的内容。
...
1. 在 **[!UICONTROL 名称]** 字段中，请键入要上传的内容名称。
1. 在 **[!UICONTROL 描述]** 字段中，键入内容的描述。 确保输入的描述有意义，长度不超过 245 个字符。
1. 要添加内容，请选择 **[!UICONTROL 添加内容文件]**，然后上传您的资源文件。 添加多语言内容时，无法将静态内容和交互式内容合并到单个组中。所有内容在所有区域设置中要么均为静态，要么均为交互式。
...
1. **Learning Manager** 支持为上传到Learning Manager的视频内容提供字幕。 现在，作者可以将包含字幕的文件连同视频文件一起上传。
   ...
   将视频内容添加到库时，也可以添加VTT文件，该文件必须是有效文件。
...
1. 在 **[!UICONTROL 持续时间]** 字段，您可以选择键入学习者将花在此模块中的预计时间。 持续时间的单位为分钟。
1. 在 **[!UICONTROL 标记]** 字段中，键入上传内容的标记，以便发现您的内容。
...
上传内容后，您可以通过选择 **铃声** 图标。 每次修改内容并重新上传后，您都会收到通知。如果您进行了更改，只有您会（其他作者不会）收到通知。

## 创建测验 {#createaquiz}

在Adobe Learning Manager中使用新的测验创建工具创建测验，该工具位于 **[!UICONTROL 内容库]** 页面。 所创建的评估将成为 **内容库** 并可添加到“公共”文件夹以便课程重复使用。

1. 选择左侧面板中的&#x200B;**[!UICONTROL 内容库]**。
1. 在屏幕右上角，选择 **添加>测验**.
1. 在 **[!UICONTROL 创建测验]** 页面中，键入测验的名称和说明。
1. 在 **[!UICONTROL 测验内容]** 部分，选择 **添加测验问题**.
1. 在 **[!UICONTROL 测试问题]** 对话框中，选择问题的类型。 问题分为 3 种类型：
   * 多选题
   * 判断题
   * 填空题
1. 键入问题，然后选择正确答案。
...
管理员创建内容文件夹后，作者可将内容上传到内容文件夹，以便内容仅对您或帐户中选定的作者组可见。 您也可公开内容，使帐户中的所有作者均可查看。
...
1. 选择要将其移至文件夹的内容。 然后选择 **[!UICONTROL 动作]** > **[!UICONTROL 整理内容]** > **[!UICONTROL 将内容移动到文件夹]**.

   ![](assets/move-content-to-folder.png)

   *将所选内容移动到文件夹*

1. 选择要将内容移至的文件夹。 选择 **[!UICONTROL 移动]**.
...
1. 要添加不同语言的内容，请选择 **添加新语言** 选项卡，然后选择所需的语言。 使用此方法，您可以添加支持多种语言的内容。
