...
1. 要保存更改，请单击&#x200B;**[!UICONTROL “保存”]**。 要发布此课程，请单击&#x200B;**[!UICONTROL “发布”]**。

## 创建课程 - 高级工作流程 {#createacourseadvancedworkflow}

1. 以作者身份登录 Adobe Learning Manager，仅作者有权创建课程。 现在，在主页上，选择&#x200B;**[!UICONTROL 创建课程]**。 此时会打开“课程目录”页面并显示课程列表。
2. 选择右上角的&#x200B;**添加**。 此时会打开一个页面以创建新课程。
3. 输入课程名称和简短描述。 该说明显示在课程卡上。 该描述不得超过 140 个字符。 然后输入课程的详细概述，该概述显示在“课程详细信息”页面上。 该概述不得超过 1,500 个字符。
...
6. 选择为课程设置价格或者将其设置为免费课程。 如果要将课程设置为付费课程，请选择相应选项&#x200B;**[!UICONTROL “付费”]**，然后指定价格。 该价格随后会显示在学习者的课程卡和课程概述页面上。

   >[!NOTE]
   >
   >配置Adobe Commerce连接器后，此选项才会启用。

7. 如果希望学习者能够取消注册您的课程，请选中&#x200B;**“学习者可自行取消注册”**&#x200B;复选框。
...
8. **实例配置**
   ...
   * 活动
   ...
   >[!NOTE]
   >
   >复制课程时，即使在源课程中启用了“实例配置”选项，但该选项在目标课程中仍处于禁用状态。
...
9. **多个注册**
   ...
   利用此功能，您可以在一个或多个时间段的多个课程实例中注册学习者。
...
15. 要保存更改，请选择&#x200B;**[!UICONTROL 保存]**。 要发布此课程，请单击&#x200B;**[!UICONTROL “发布”]**。

## 为课程配置Gradebook {#configuregradebookforacourse}
...
1. 选择&#x200B;**添加模块**。
2. 在模块选择器中，选择要添加的模块并选择&#x200B;**添加**。 模块显示在&#x200B;**内容**&#x200B;部分。 可评分模块、SCORM、Captivate内容、AICC、xAPI、本机测验、活动模块、教室会话和虚拟教室会话，显示&#x200B;**权重**&#x200B;输入字段。 不可评分模块在权重列中显示破折号。
3. 在每个可评分模块的&#x200B;**权重**&#x200B;字段中输入百分比值。 **总重量**&#x200B;指示器会随着您键入而更新，必须正好达到&#x200B;**100%**&#x200B;才能保存。

   ![](assets/image_0004.png)

4. 对于具有多个传递类型的模块：仅当模块中的&#x200B;**所有**&#x200B;传递类型都支持评分时，才能分配权重。 如果任何一个交付类型不支持评分，则无法对整个模块进行加权。
...
## 创建实例 {#createaninstance}

要创建实例，请执行以下操作：

1. 以&#x200B;**作者**&#x200B;身份登录。
2. 导航至要为其创建实例的课程并打开课程。
3. 在左侧导航面板中选择&#x200B;**实例**。 将打开&#x200B;**实例**&#x200B;页。 默认情况下，每个课程都有一个实例。 您可以修改默认实例或添加实例。 您无法删除此课程实例。
4. 要创建示例，单击课程信息右上角的&#x200B;**[!UICONTROL “添加新实例”]**。 此时会显示课程的新实例。
5. 输入实例的属性：
   ...
   * 选择&#x200B;**[!UICONTROL 显示更多选项]**&#x200B;以显示其他截止日期选项。
...
只有在保存整个实例信息后，才能设置电子邮件提醒。 选择“**保存**”后，实例会自我最小化并进入编辑模式。
...
2. **会话：**&#x200B;课程中的虚拟教室和教室模块数
3. **成就：**&#x200B;学习者的成就
4. **通知警报：**&#x200B;在课程开始日期之前发送两封提醒电子邮件的选项

### 操作 {#actions}

* **保存到课程设置：**&#x200B;将此实例的设置保存到从中创建实例的课程。
* **应用默认设置：**&#x200B;应用在&#x200B;**配置** > **实例默认值**&#x200B;页面中找到的默认设置。
* **停用实例：**&#x200B;已停用实例。 该实例将不再可用。
* **删除实例：**&#x200B;删除特定实例。
...
>[!NOTE]
>
>不能删除默认实例或默认实例的副本。 在这种情况下，“删除实例”选项将不会显示在“操作”下拉列表下。

* **通知轮候学习者：**&#x200B;通知轮候学习者存在其他可以注册的实例
* **向注册的学习者发送电子邮件：**&#x200B;向学习者发送有关重要公告的电子邮件
...
管理预订讲师和教室中的冲突。 向课程分配讲师之前，如需了解任意讲师的可用时间和日期，请使用“计划助手”。

创建 VC 或 CR 课程时，请单击“计划助手”。
...
只能按部分或完整的技能名称搜索技能，而不能按技能级别进行搜索。

在“计划助手”中，输入讲师姓名、位置和名额限制。

此外，您还可以搜索技能。单击讲师搜索框右侧的筛选图标后会显示技能。 您可在下方的屏幕截图中看到该按钮。
...
创建课程时，您可在“课程概述”字段中看到富文本编辑器。 您可以设置内容格式，及添加图像和超链接等。
...
1. 单击&#x200B;**[!UICONTROL “添加模块”]**。 您可以看到四个添加模块的选项。 第一个选项是添加自学模块。 这些是您创建并添加到 Adobe Learning Manager 模块库的模块。 第二个选项是设置虚拟教室。 第三个是设置教室模块，第四个是活动模块。
   ...
   **教室模块：**&#x200B;在该模式下，学习者可以参加面对面讲座，并由受过培训的讲师提供协助。 输入标题、描述并设置会话的持续时间。 您还可指定教室的位置和执行会话的讲师。 要保存更改，请单击&#x200B;**[!UICONTROL “完成”]**。
   ...
   >[!NOTE]
   >
   >能否在此处创建新位置取决于管理员配置的“允许作者创建位置”设置。 如果禁用此设置，则只能搜索并选择管理员已添加的位置。 查看[添加教室位置](../../../migrated/administrators/feature-summary/classroom.md)以了解更多信息。

   ![](assets/classroom-module.png)

   *添加教室模块*

   在通过虚拟教室配置对话框创建课程时，请将会议系统与您创建的 Microsoft 团队连接相关联。 选择是否要为事件设置会议组织者。

   如果选择是，即需为事件设置会议组织者，则必须输入组织者的名称。 输入组织者的名称并选择组织者。
   ...
   **注意**：如果学习者希望以访客身份加入 Microsoft Teams，则必须输入电子邮件。 电子邮件必须存在于 Adobe Learning Manager 中。
   ...
   *添加活动模块*
   ...
   在课程中就活动类型“文件提交”添加活动模块及添加基于 xAPI 的模块时，您可指定持续时间。
...
1. 选择“**清单**”选项后，您还会看到一些其他选项。

2. **清单类型：**&#x200B;请选择任一选项：**是/否**&#x200B;或 **1-5**。 如果您选择“是/否”，则清单将包含只能回答“是”或“否”的问题。 如果选择“1-5”，您将看到 Likert 清单，并需要按照五分制对问题进行评分。

3. 选择&#x200B;**配置**。 此时将出现“清单”窗口。
4. 输入您想要的问题。 您也可以使用其他语言输入相同的问题。
   ...
   *例如，问题也以法语输入*
   ...
   >[!NOTE]
   >
   >ALM不会自动为您翻译这些附加语言的问题。 你必须把问题翻译成相应的语言。

5. 选择&#x200B;**“保存”**。 系统会以所有语言保存问题。
6. 选择所有其他相关选项并选择&#x200B;**添加**。 该核对表会以该课程提供的所有语言编写。
...
清单注释允许作者在基于清单的评估期间启用&#x200B;**上下文反馈**。\
启用后，审阅者（讲师或经理）可以添加备注，说明评分结果超出分数或通过/未通过状态。

此功能可将核对清单从纯粹的评估工具转换为&#x200B;**反馈驱动的学习机制**，从而改善学习者理解、透明度和后续操作。
...
* 控制注释对学习者是否可见
* 控制是否向学习者显示审阅者姓名
* 确保将审阅者的注释捕获到学习者成绩单和报告中

#### 用例

#### 业绩评价

在真实世界或基于角色的评估过程中，就观察到的优势、差距和改进领域提供明确的反馈。

#### 讲师指导和VILT培训

让讲师解释学习者通过或未能通过基于清单的评估的原因。
...
帮助学习者确定评估者并进行后续追踪以澄清问题。

#### 启用清单注释

1. 打开现有课程或创建新课程。
2. 添加清单模块或打开现有清单模块进行编辑。
3. 在清单配置面板中：
   1. 选择&#x200B;**审阅者注释**。 审阅清单时，审阅者可以添加注释或反馈。 发布后，此设置无法更改。
   2. （可选）选择&#x200B;**向学习者显示审阅者姓名**。 学习者可以查看审阅清单模块的审阅者的姓名。
4. 保存清单配置。
5. 在Publish课程中公布评估清单。

清单现在支持在学习者评估期间提供审阅者评论。

#### 评估过程中发生的情况

虽然作者不执行评估，但了解如何使用配置会很有帮助：
...
这有助于学习者清楚地了解评估结果和后续步骤。

#### 面向作者的重要说明

* 清单注释是&#x200B;**可选的**，必须明确启用
* 注释和审阅者标识的可见性&#x200B;**完全由作者控制**
* 适用于：
  * 单个或多个审阅人
  * 基于分数、是/否和缩放检查清单
* 除非作者更新，否则不会对现有清单产生影响

### 创建多语言清单

对清单的多语言支持允许作者创建&#x200B;**单个清单模块，其中包含以多种语言提供的问题**。\
启用后，审阅者会自动在其&#x200B;**所选内容语言**&#x200B;中看到清单问题，从而确保全球团队的评估一致且本地化。

这样便无需为每个语言创建和管理&#x200B;**单独的检查单**，从而减少了重复并简化了课程维护。

借助多语言清单支持，作者可以：

* 在同一清单中添加多种语言的清单问题
* 跨清单问题保持语言均等
* 确保审阅者看到已自动本地化为其内容语言的清单问题
* 用所选语言下载清单报告
...
允许各地区的经理和讲师使用本地化的清单问题对学习者进行评估。

#### 跨区域标准化评估

保持单一评估结构，同时支持不同地区的多种语言。

#### 法规遵从性和运营培训

确保评估人员清楚了解其母语清单标准，从而减少解释错误。
...
1. 打开现有课程或创建新课程。
2. 添加清单模块。
3. 在课程设置中，添加所需的&#x200B;**内容语言**（例如，英语、法语、德语）。
4. 保存课程配置。
...
1. 打开清单模块配置。
2. 导航至&#x200B;**配置问题**&#x200B;部分。
3. 使用&#x200B;**语言选项卡**&#x200B;在可用语言之间切换。
4. 对于每种语言：
   * 添加每个清单问题的翻译版本
   * 维护不同语言之间相同的含义和评估意图
5. 根据需要定义核对清单标准（“是/否”、“基于等级”或“基于评分”）。

每个语言版本表示相同的清单，而不是单独的模块。

保存清单配置并发布课程。 现在，该清单已提供所有已配置的语言版本。

#### 审阅者的体验

* 审阅者会看到&#x200B;**他们所选内容语言**&#x200B;的清单问题
* 评估过程中不需要手动语言切换
* 如果审阅者更改了内容语言，则清单问题会自动调整

### 为清单问题指定权重

清单问题权重允许作者创建&#x200B;**基于分数的清单**，其中单个清单问题可以包含&#x200B;**不同的最高分数**。

作者可以定义加权问题，并设置学习者需要通过的&#x200B;**最低总分**，而不是对所有清单项目一视同仁。

此功能可实现更准确、更灵活的评估，特别是对于&#x200B;**基于性能、以评估为导向和以合规性为导向的培训**。
...
使用详细评分标准而不是简单的是/否结果来评估学习者。

#### SOP和操作培训

为关键步骤分配较高的权重，为次要任务分配较低的权重。

#### 法规遵从性和认证就绪性

确保学习者在通过之前满足规定的最低能力阈值。

#### 技能和性能基准设定

支持结构化的评价，其中部分完成仍然有助于总体评分。

#### 如何创建加权清单

1. 打开现有课程或创建新课程。
2. 添加清单模块或打开现有清单模块进行编辑。
3. 在清单配置面板中，选择&#x200B;**自定义评分**&#x200B;清单类型。
4. 这样将启用每个问题的评分和总分计算。
5. 根据需要添加清单问题。
6. 对于每个问题：
   * 定义&#x200B;**最大分数**（例如，5， 10， 20）
...
同一份清单中的不同问题的最高分数可能不同。

#### 定义通过标准

1. 指定通过清单所需的&#x200B;**最低总分**。 系统根据试题权重自动计算总可能得分。
2. 保存清单配置并发布课程。
...
* **已删除** — 课程从Adobe Learning Manager应用程序中完全移除后，即显示为删除状态。 只有作者能够删除处于“草稿”或“已弃用”状态的课程。
...
1. 单击个人资料图片，然后选择“管理员”。
1. 在“管理员”模式下，单击左窗格中的&#x200B;**[!UICONTROL “课程”]**。 在此页面上，您可以查看 Adobe Learning Manager 帐户中所有作者创建的所有课程。
...
1. 在“包含学习者”字段中输入学习者姓名，然后选择学习者。 您还可在此处添加用户组。 如果希望注册您的 Adobe Learning Manager 帐户中所有的学习者，则键入“全部”。 您还可注册团队中的学习者。
...
在&#x200B;**“我的课程”**&#x200B;中，作者可以在课程缩略图上查看课程效果评级，如下图所示。 您可以看到此课程的评分为 100。
...
1. 在新创建的“目录”选项中，您可以选择可添加的不同学习类型，例如学习计划、证书或课程。
...
**先决条件：**&#x200B;管理员必须在管理员应用程序中的&#x200B;**“设置”>“常规”**&#x200B;中启用&#x200B;**“多次尝试”**。
...
不能，已上传至 Adobe Learning Manager 的内容是已发布的压缩文件，而不是源文件。 因此，即使下载了内容，也无法在创作工具中进行编辑。 您需要源文件来编辑内容。
