...
1. 此时会显示一个对话框，提示您输入电子邮件 ID。 提供负责管理公司 Adobe Learning Manager FTP 帐户的人员的电子邮件 ID。 提供电子邮件 ID 后，单击&#x200B;**[!UICONTROL “连接”]**。
...
如要打开已导出文件位于 FTP 中的“导出”文件夹，请打开“用户技能”页面中提供的“FTP 文件夹”链接，如下所示：
...
**先决条件**：请求公司的 Workday 管理员创建具有 ISU_Permissions 文档中定义的权限的集成系统用户 (ISU)。 从下面的链接下载的一个副本。
[下载集成系统用户(ISU)安全性的副本。](assets/isu-permissions-v1.pdf) 了解如何将 Adobe Learning Manager 与 Workday 连接器集成。
...
Workday有四个级别，而Learning Manager有两个级别。 Workday 中的四个级别分别时技能配置文件类别、技能配置文件、技能项目类别和技能项目。 您的技能名称以及Learning Manager的级别将一同映射到Workday的技能项目之下。
...
您可以将出用户达成的全部技能从 Adobe Learning Manager 导出到 Workday。 请注意，Adobe Learning Manager 仅会导出所有活动用户技能，不会导出弃用的技能。 您还可以将多个 Adobe Learning Manager 帐户连接到同一个 Workday 连接器。 如果两个 Adobe Learning Manager 帐户存在名称相同的技能，则它们会被映射到 Workday 下同一项技能中。 如果两个Learning Manager帐户使用的是同一个Workday帐户，则建议先更新所有Learning Manager帐户中的技能名称，然后再更新Workday中的技能。
...
1. 如果要直接将 miniOrnage 用户导入成为 Adobe Learning Manager 内部用户，请使用&#x200B;**[!UICONTROL 导入内部用户]**&#x200B;选项。
...
您现在可以将 Adobe Learning Manager 与 BlueJeans 连接器集成，并使用 BlueJeans 来托管课程。 您可以利用 BlueJeans 发起音频和视频电话会议、视频聊天和网络研讨会。
...
1. BlueJeans 连接器页面即会打开。 在相应字段中输入您帐户的详细信息，以将 Adobe Learning Manager 与 BlueJeans 集成，实现用户订阅源的同步。 您可以从您 BlueJeans 帐户的管理员处获取这些详细信息。
...
应由客户负责管理访问权限和 Adobe Learning Manager 团队在 Box 文件夹中共享的内容。  另请注意，文件夹中的内容将物理存储在法兰克福地区。
