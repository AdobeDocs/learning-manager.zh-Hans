...
当更改数小于记录总数时，最好在一个新的 Sprint 中运行修改后的 CSV。
...
在创建Sprint之前，请确保将当前Sprint标记为完成。 单击页面顶部的&#x200B;**[!UICONTROL 将 Sprint 标记为完成]**&#x200B;可完成 Sprint 迁移。

### 无法将“迁移项目”标记为完成，因为该按钮已禁用 {#unabletomarkamigrationprojectascompleteasthebuttonisdisabled}

在标记迁移项目完成之前，请确保将当前Sprint标记为完成。 单击页面顶部的&#x200B;**[!UICONTROL 将 Sprint 标记为完成]**&#x200B;可完成 Sprint 迁移。

## CSV问题 {#csvissues}

### module_version.csv 文件迁移失败且内容尚未迁移 {#moduleversioncsvfilemigrationisfailingandcontentisnotmigratedyet}

确保内容在内容文件夹（指定迁移项目、Sprint路径下的Box帐户）中可用。 此外，请确保您已为&#x200B;**是否将迁移此Sprint的内容**&#x200B;选择了选项&#x200B;**是**？ Sprint创建页面中的问题。
...
确保课程的 **moduleOrderInCourse** 键值从 **0** 开始，并连续排列。 courseModuleType的顺序应为PRETEST、TESTOUT、CONTENT
