...
**视频缩略图：**&#x200B;学习者现在可以按图块快速识别视频，而图块会显示缩略图，帮助读者快速识别视频内容。 如果源视频没有提供自己的缩览图图像，Adobe Learning Manager会使用通道主题颜色、抽象图案和视频标题自动生成一个缩览图图像。

**频道：**“频道”导航选项仅在以下情况下对学习者可见：已创建至少一个频道，其中至少包含一个来自其源的视频，并且已被标记为对学习者可见。 详细了解[频道](/help/migrated/administrators/feature-summary/create-channels.md)。

## 已修复的错误

**Adobe Experience Manager组件：**&#x200B;升级到Adobe Learning Manager参考站点包v2.0.1后，当用户单击通知铃铛时，“通知”面板加载失败。 加载器继续无限期旋转，防止显示通知。 此问题还导致对/userNotifications终结点的重复调用，最终导致HTTP 429响应。 修复了通知加载工作流程，以防止出现过多的API请求并解决加载问题。 现在，通知可以正确加载，面板会按预期打开，而不会出现不必要的API限制。

**Adobe Learning Manager移动应用程序>学习路径：** Adobe Learning Manager移动应用程序中已完成的课程在学习路径中错误地显示为黄色指示符和红点，即使其状态标记为已完成。 只有学习路径内的课程在正确显示已完成的模块时，才会出现视觉上不一致。 移动应用程序中的“学习路径”课程的状态指示器逻辑已修复。 已完成的课程现在会显示正确的绿色完成指标。 这确保在移动应用程序和桌面浏览器之间实现一致的完成体验。

+++


+++更新110：Adobe Learning Manager 2026年8月16日版
...
在AI方面，学习者的AI Assistant从Go1和LinkedIn学习目录中获取课程摘要、学习对象比较、Adobe Experience League答案和第三方内容查询；新的学习路径代理会引导学习者通过对话自动生成自定义的序列学习路径；而Insights代理会为管理员报告提供自然语言查询。 所有AI功能都受“帐单”页面中新的Gen AI积分系统的控制，为管理员提供了针对每项功能的使用限制和使用警报。
...
这些操作会将特定于帐户的“外部学习”自定义字段添加到学习者成绩单(LT)报告。 在管理员学习者成绩单中，新的自定义字段列会附加到末尾；在学习者学习者成绩单中，外部学习名称和完成注释会插入相关模块和审阅者字段的附近，其余自定义字段会添加到末尾。

详细了解[报告Adobe Learning Manager 2026年8月版中的更改](/help/migrated/reporting-changes-august-2026.md)。

### 用于学习者成绩单中认证的根培训ID

这会将新的根培训ID列添加到管理员学习者和学习者学习者学习中。 该列显示在报告末尾，可帮助确定学习者记录的父认证上下文。

详细了解[报告Adobe Learning Manager 2026年8月版中的更改](/help/migrated/reporting-changes-august-2026.md)。

### Webhook日期 — 时间格式与学习者成绩单对齐

这将Webhook数据对象的日期 — 时间值标准化为分钟精度。 秒始终以00形式发出，将Webhook时间戳与LT报告格式对齐。
...
详细信息会更新共享课程的learningObjects API学习对象响应，以便接收帐户不再将接受管理员显示为作者。 配对帐户中仅公开原始外部作者详细信息；父帐户行为保持不变。

### AI Orchestrator代理中的意图检测

AI Orchestrator代理会将单个查询请求的目的检测移入Orchestrator并删除手动用户界面切换。 查询路由可能会在Q&amp;A流和PLP流之间发生变化，具体取决于检测到的意图。

## 版本中修复的错误

**学习者成绩单：**&#x200B;学习者成绩单报告中的日期过去仅记录00秒，但在Webhook中，秒数按预期显示（两位数）。 此问题已修复。 学习者成绩单现在还可以正确显示两位数的秒数。

**实例：**&#x200B;当从实例页面取消某个Microsoft Teams或Adobe Connect VC会话时，刷新页面后仍保留一些关联的会话详细信息。 此问题已修复，取消的会话现在可正确删除所有关联详细信息，包括“团队组织者”、“绕过大厅”设置以及“连接”主讲师。

**API：**&#x200B;自动AWS工作流程中存在一个问题，该问题导致向CMS HTTP API发出的重复下载请求使用了编码错误的URL，从而导致HTTP 500出现间歇性错误并产生大量无效流量。 更新了工作流程，以正确对下载请求URL中的空格和其他特殊字符进行URL编码。 结果，帐户资产下载请求得以成功处理，从而减少了错误并提高了自动资产检索的可靠性。

**学习者：**&#x200B;发布后，某个问题会导致应用于课程“详细概述”部分的斜体格式在创作期间正确显示，而不会显示在面向学习者的课程概述页面上。 更新了渲染逻辑，以在发布课程内容时保留斜体文本格式。 因此，对于发布课程概述页面上的学习者，详细概述部分应用的斜体可以正确且一致地显示。

**自定义认证：**&#x200B;上传SVG图像作为成就证书的证书背景时，证书预览显示空白背景。 已更新证书预览渲染过程，以正确支持和显示上传的SVG背景图像。 因此，证书预览现在可按预期渲染配置的SVG背景，从而提供最终证书的准确表示。

**作者：**&#x200B;为长时间交互式视频生成VTT文件可能会遇到严重延迟，某些上传的处理时间可能会超过两个小时。 针对此问题，进行了研究和优化，以提高大型视频文件的VTT生成性能和可靠性。 因此，现在可以更高效地生成VTT文件，为交互式视频上传提供更快、更一致的字幕工作流程。

**API：**&#x200B;报告导出API未返回某些学习者完成记录，即使后端中存在完成数据，与指定的日期范围和用户组筛选条件匹配并符合所有报告条件也是如此。 已改进跨多个作业和数据检索逻辑的学习者完成情况报告导出流程，以确保在API导出中始终包含符合条件的完成记录。 报告导出API响应现在可为完成时间戳位于请求的日期范围和过滤器范围内的用户返回完整的学习者完成数据，从而提供更准确可靠的报告结果。

**Webhook：**&#x200B;当学习者注册了后来取消的会话时，注册同一课程的另一会话未触发新的注册Webhook事件，除非该学习者首先从取消的会话中取消注册。 这会导致下游系统错过已取消或以前完成的实例的有效注册更新。 Webhook流已更新，因此当学习者在较早的会话取消或完成后注册同一课程的其他会话时，会触发新的注册事件，以确保注册活动得到一致的报告。

**API：**&#x200B;针对包含大量相关数据（如实例、注册、资源和等级）的学习路径的管理员API请求遇到了性能显着下降的问题，响应时间从通常的5秒到8秒增加到了多达45秒到60秒，进而影响了集成工作流程和下游系统。 已实施性能优化，以包括大量学习对象请求，从而提高数据检索效率。 包含复杂嵌套关系的学习路径API调用现在可以更一致地返回结果，并缩短响应时间，帮助确保可靠的集成性能。
...
**API：**&#x200B;学习者按名称升序(A-Z)排序目录时，公共API返回的结果区分大小写，导致大写课程名称(AA、BB、CC)出现在小写名称(aa、bb)之前，而不是按照标准字母顺序排列。 排序逻辑已更新为使用不区分大小写的比较。 现在，目录结果会按预期的字母顺序显示，而不考虑字母的大小写，从而确保课程名称的顺序一致。

**CR-VC：**&#x200B;在将新讲师添加到虚拟教室课程时，如果禁用“您已作为讲师/组织者/共同组织者添加”电子邮件模板，则不会发送会话邀请电子邮件，从而使讲师无法接收会话详细信息。 已更正通知逻辑，以确保教师会话邀请的生成与模板无关。

**客户端常用：**&#x200B;在将课程添加到自动注册触发器(AET)学习计划后更新活动模块的持续时间时，学习者预览会继续显示AET实例中的持续时间，而不是默认课程实例中更新的持续时间。 此问题已修复，确保学习者预览从正确的课程实例检索模块持续时间。 学习者预览现在可以准确显示最新发布的模块持续时间，反映对课程内容所做的更新。
...
**电子邮件模板和配置：**&#x200B;当管理员更新电子邮件模板设置中的电子邮件横幅时，新横幅在Outlook桌面中正确显示，但在Outlook Web中未显示，导致客户端之间的电子邮件体验不一致。 电子邮件模板呈现已更新，以确保跨Outlook平台的横幅兼容性。 更新的电子邮件横幅现在可以在Outlook桌面版和Outlook Web版中正确显示。
...
### “积分持续时间”字段输入上限为9999.99

当用户在会话编辑表单中输入大于9999.99的“积分持续时间”值（如999999）时，系统会在保存后静默将该值限制为9999.99，并且不会向用户显示验证错误或警告。 该字段的HTMLmax属性设置为9999.99，在浏览器级别强制执行此上限，而不通知用户其输入已被拒绝或修改。 此版本未应用任何修复，因为此行为已确认可以正常工作。

### 错误消息不指示CSV上传中缺少默认语言行

当结构化位置CSV上传仅包含采用非默认区域设置（例如fr、de）的行，并且没有采用帐户默认语言的行时，返回的错误消息是CSV第1行中的区域设置，第2列无效或不受支持。 这并不表明实际的问题是缺少默认语言行。 这样可防止用户上传多区域设置CSV，并防止仅通过错误消息诊断问题。 此版本已删除此问题，未应用修复。
...
+++更新109：Adobe Learning Manager 2026年7月版

发行日期：2026年7月16日

## 已修复的错误

**电子邮件模板：**&#x200B;在未提供课程日期和时间信息的情况下向已注册课程的学习者发送了近期课程的提醒电子邮件。 此问题已修复，以在电子邮件模板中正确填充会话计划详细信息。 学习者现在会收到包括相关会话日期和时间的前期会话提醒电子邮件，可提供完整的日程安排信息。

**连接器：**&#x200B;作者在复制或编辑基于连接器的课堂/虚拟教室课程时间歇性地遇到HTTP 400错误。 触发后，问题将持续存在，并且会阻止后续的课程创建或编辑操作，直到刷新会话为止。 此问题已修复，可确保基于连接器的课程得到可靠的复制和编辑。 作者现在可以复制、创建和修改教室/虚拟教室课程，而不会遇到重复的400个错误。

**CMS API请求编码：**&#x200B;自动化平台工作流正在生成针对CMS HTTP API的重复下载请求，其中具有编码不正确的URL，导致间歇性HTTP 500错误，有时每秒钟有数千个无效请求。 已更正URL编码逻辑，以确保在发送请求之前，对包含特殊字符（如空格）的资源路径进行正确编码。 现在可以成功执行到NetStorage的下载请求，从而消除重复出现的API错误和请求高峰。

**翻译工作流程可复原性：**&#x200B;由于基础翻译服务不支持语言对，从英语到挪威博克马尔语的翻译请求失败，导致整个翻译工作流程中断。 翻译处理逻辑已得到增强，可以轻松管理不受支持的语言对，并隔离单个翻译故障。 现在，不支持的语言的翻译请求会被跳过并相应报告，而其余所有受支持目标语言的翻译将继续得到成功处理。

+++

+++更新108：Adobe Learning Manager 2026年6月版
...
**已获取的课程在重复认证中的可用性：**&#x200B;以前，已获取的课程只能添加到永久认证。 循环认证不会在课程选择面板中显示已获取的课程，导致接收者帐户无法将共享内容用于需要定期续订的课程。 已获取的课程现在可通过永久认证和重复认证获取。 有关详细信息，请参阅[接收方应用程序中的共享目录功能](/help/migrated/administrators/feature-summary/catalogs.md#sharedcatalogfeaturesinreceiverapplication)。

**将VILT会话迁移到Adobe Learning Manager：** Adobe Learning Manager现在支持通过CSV文件批量迁移和更新虚拟讲师指导培训(VILT)会话数据。 使用此工作流程配置实例开始日期、将学习路径实例与课程实例相关联以及设置Microsoft Teams、Adobe Connect和Zoom的虚拟教室会话。 有关详细信息，请参阅[迁移手册](/help/migrated/integration-admin/feature-summary/migration-manual.md#migrationofviltsessiontoalm)

## 此版本中的增强功能

**李克特刻度顺序颠倒：**&#x200B;在Adobe Learning Manager中，李克特刻度用于对学习者的学习对象体验进行评分。 “学习者体验反馈”表单上的李克特量表已反转以匹配标准的从左到右顺序。 以前，李克特量表按从右到左的顺序排列，这和标准的从左到右的顺序相反。 现在，分数从左侧的“强烈不同意”开始，到右侧的“强烈同意”结束。

**讲师向学习者发送的临时电子邮件：**&#x200B;以前，“向全体学习者发送电子邮件”选项仅允许讲师复制收件人的电子邮件地址。 讲师现在可以使用同一选项直接向学习者发送临时电子邮件。 复制电子邮件选项仅可用于复制收件人的电子邮件地址。

### 步骤

1. 以讲师身份登录。
2. 导航到左侧导航面板上的“过去的会话” 。 页面中会显示过去会话的列表。
3. 从过去会话列表中选择会话名称。
4. 在左侧导航面板中导航至学习者。
...
1. 在自动生成VTT字幕文件时，翻译质量分数较低的区域设置不再显示在字幕生成模式中。 这样可以防止向学习者提供低质量的翻译，例如不准确的中文字符渲染。

2. 欢迎电子邮件（内部）模板现在具有关于Adobe ID和SSO登录方法的独立配置。 以前，这两种登录方法共享一个模板通道，导致在禁用模板后配置SSO时，该模板会自行重新启用，从而导致向学习者发送无意的欢迎电子邮件。

**认证**

循环认证现在从认证的最新（父）版本而不是原始（根）版本获取课程。 这样可确保更新后的课程内容正确地纳入每个认证周期。

**内容库和自定义角色**

1. 访问权限仅限于选定内容文件夹的自定义作者现在可以使用“以学习者身份预览”选项预览AICC和Tincan课程模块，而不会收到“内容不可用”错误。

2. 有权访问选定内容文件夹和目录的自定义作者现在可以在“预览为学习者”视图中打开课程资源，而不会遇到403访问错误。
...
公共&#x200B;**搜索**&#x200B;和&#x200B;**类别** API现在会以正确的数字格式返回目录ID。 以前，目录ID是以格式错误的字符串格式“catalog123”返回的，而不是所需的数字ID。

**UI和管理**

1. 经理指派的课程的“课程实例”页面不再为管理员和自定义管理员用户显示损坏的布局。 已更正导致元素溢出但不换行的CSS回归。

2. 现在，单击电子邮件中的VILT会话邀请链接的外部用户可正确重定向至其配置的登录方法（例如Adobe ID），而无需错误地路由至内部SSO登录页面。

**AI助手**

当被问及能否将学习路径添加到认证时，AI Assistant现在会提供正确的响应。 助理以前返回了不正确的肯定答案；现在它正确声明，只有课程才能添加到Adobe Learning Manager中的认证。

## 已知问题

**基于QR的课堂出勤在学习者成绩单上花费的时间：**&#x200B;对于通过QR代码自标出勤的学习者，花费的时间显示为0分钟，并确认可以正常工作。 “学习者成绩单”中记录的会话持续时间源自完成时发出的xAPI数据。 如果在处理完成时尚未向数据库提交会话计划数据，则会记录零持续时间。 此版本未应用任何修复。

+++

+++更新107：Adobe Learning Manager 2026年5月版

发行日期：2026年5月26日

## 此版本中的增强功能

**实例的开始日期：**&#x200B;作为管理员或作者，您现在可以在创建或编辑实例时包含实例的开始日期。 开始日期只能通过API显示在学习者界面中。
...
Adobe Learning Manager 2026年4月版引入了学习者体验、管理、内容创作和报告方面的增强功能，其中一些功能目前处于测试阶段，可能会发生变化。

**学习者体验增强功能**

* 改进了流体播放器导航，带有清晰的“下一个模块”标签和专用退出操作，以更好地安排课程流程。
* 多语言支持扩展到工作辅助、清单和视频字幕。
* AI Assistant集成到学习体验中，以提供上下文帮助。
* 通过统一的目录、完成指标和改进的注释导出增强了Captivate内容体验。
* 支持通过LTI设置播放器语言，以获得一致的跨平台体验。

**管理员和作者更新**

* Zoom连接器现在支持多个并发虚拟会话，从而提高了调度效率。
* 可通过API限制模块启动时间并提高学习对象到期情况的可见性。
* 通过加权评分、多语言支持和审阅者反馈选项增强了清单模块。
* 使用拖放编辑器、动态字段和AI生成的背景升级的自定义证书。
* 引入了一个用于创建公共学习页面的未登录Experience Builder。
* 共享的课程现在可正确显示原始作者。
...
* 等效项和替代项的增强功能可启用更准确的课程替代和映射，从而提高学习路径的灵活性。

**主要影响**
这些更新提高了可用性、可扩展性和自动化，重点关注AI辅助学习、多语言支持、简化的虚拟培训管理和更准确的报告。

查看[Adobe Learning Manager 2026年4月版的新增功能和更改](/help/migrated/whats-new.md)，了解更多信息。

## 此版本中修复的错误

**高级搜索：**&#x200B;当缺少目录访问时，高级搜索不接受基于用户注册的学习对象访问权限。 现在，“高级搜索”将同样的基于注册的访问检查应用为词法搜索，从而允许注册用户访问学习对象，而无需访问目录。

**Adobe Experience Manager：**&#x200B;目录小组件标题未保留所选目录并打开包含所有学习对象（而不是筛选后的目录）的目录页面。 现在，单击目录小组件标题可打开目录页面，其中已预先选择相应的目录，并正确过滤为该目录的学习对象。

**设备应用程序移动版：**&#x200B;在移动设备上，学习者可以完成有序学习路径第一部分中的所有项目，但第二部分在成功完成之后也未解锁。 在移动设备上完成部分中的所有项目后，有序学习路径中的下一部分现可为学习者正确解锁。

**LMS迁移：**&#x200B;通过迁移创建的课程实例具有字母数字实例ID时，重新运行迁移以更新VC会话将创建新会话，而不是更新现有会话。 现在，迁移过程可使用字母数字ID正确更新课程实例的现有VC会话，而无需创建重复会话。

**课程持续时间：**&#x200B;对于VILT课程，课程级别的持续时间始终取自默认实例上的静态LO_DURATION，当会话停用时，该持续时间会变为0，并且不会从实际实例模块重新计算。 VILT课程持续时间现在反映实际实例模块，而不是仅依赖默认实例中过时或零的LO_DURATION值。

**连接器：**&#x200B;由于Ember模块详细信息控制器已在各个模块间重复使用，因此vcHostingSystem未得到一致更新，从而导致出现混合VC/F2F行为，并且仅基于vcHostingSystem错误地清除instructorId。 模块处理现在可保持vcHostingSystem的一致性，并仅清除真正虚拟教室会话的instructorId，以便正确更新已转换的F2F会话的讲师。

**LMS迁移：**&#x200B;后端未遵守课程顺序的LearningProgramCourse CSV列，因此即使orderEnforced为True，也会忽略LP部分中的课程顺序。 已删除课程订单中未使用的CSV列，以匹配后端行为，避免通过CSV显示不支持的订购。

**性能：** AWS自动化工作流程向Akamai CMS HTTP API发送了反复编码错误的下载请求（带有未编码的空间），导致突发出现500个错误和过多的无效流量。 现在，自动化工作流程可正确对下载URL中的空格进行URL编码，从而防止无效请求并消除关联的500错误高峰。

**注册和取消注册：**&#x200B;在某些学习计划、认证和课程的管理注册流程中，注册源被错误记录为SELF_ENROLL而不是ADMIN_ENROLL。 管理员驱动的注册现在与源ADMIN_ENROLL在受影响的流中一致存储。

**注册和取消注册：**&#x200B;对于认证循环链中基于阈值的注册，即使学习者尚未完成上一个循环，他们也会注册到以后的循环中。 现在，基于阈值的注册会遵守重复链，学习者除非已完成所需的先前重复，否则不会注册以后的认证。

**管理员设置：**&#x200B;管理员无法添加其子域以数字开头的配对帐户，因为验证规则要求子域以字母开头。 现在接受子域以数字开头的配对帐户，只要这些帐户符合允许的字符规则。

**课程 — 测验分数：**&#x200B;在无头启动中，管理员重置测验模块后，播放器显示错误的总共12个问题，而不是实际的3个问题。 在无头启动中，测验播放器现在会在模块重置后显示正确的总问题数。

**作者：**&#x200B;将内容组从SCORM更新为CAPI时，未刷新报告信息和完成条件，使内容组没有完成条件。 现在，将内容组从SCORM更新到CAPI可正确更新报告信息并为内容组设置相应的完成标准。

**设备应用程序移动版：**&#x200B;在Captivate Prime移动应用程序中，通过target=&quot;_blank&quot;在新选项卡中打开的可下载资源在点击时未下载。 在移动应用程序中，现在点击可下载的资源即可正确启动下载，即使内容使用target=&quot;_blank&quot;链接也是如此。

**富文本编辑器：**&#x200B;发布后，尽管学习者仍看到换行，但作者视图中删除了富文本学习对象描述中的换行符。 现在，富文本学习对象描述可在作者视图和学习者视图中正确保留和显示换行符。

## 已知问题/按设计

### Second Nature资源的版本控制行为

在编辑场景中，Adobe Learning Manager (ALM)不会递增第二自然(SN)资源的版本。 此行为反映了ALM处理SN内容更新的方式而不是创作选择。
...
+++更新104：Adobe Learning Manager中学习者的AI助手

发行日期：2026年3月2日

## 面向学习者的AI助理

GenAI支持的对话工具，旨在帮助您的学习者使用自然语言查询即时查找回复、资源和相关课程。
...
+++更新103：Adobe Learning Manager 2025年11月版
...
Experience Builder现已面向所有Adobe Learning Manager帐户提供，使客户能够使用低代码组件和灵活的布局创建完全品牌化的学习体验。 管理员可通过Experience Builder创建完全对移动设备响应的自定义学习门户和页面。

对于现有帐户，Experience Builder更新将自动推出。 对于继续使用旧UI或经典UI的帐户，Experience Builder功能不可用。

有关详细信息，请查看[Experience Builder](/help/migrated/administrators/feature-summary/experience-builder/overview.md)。

### Adobe Learning Manager移动Web界面

移动Web界面已在现代代码库的基础上进行了重建，从而加快了加载速度、改善了响应速度并在不同设备上提供更直观的学习者体验。

默认情况下，会为所有新帐户启用更新的移动Web体验。 除启用“社交学习”外，现有帐户也将自动收到升级。 在这种情况下，先前的移动体验将保持活动状态，直到社交学习迁移到新的兼容框架。

>[!NOTE]
>
>新的Adobe Learning Manager移动Web界面不支持社交学习。

### 流体播放器增强功能

以前，流体播放器中的“下一步”按钮不显示即将开始的模块的标题，因此学习者很难预测学习旅程中的下一步。

通过此增强功能，“下一步”按钮现在包含下一模块的标题，使得学习者可以：
...
_流体播放器现在显示“下一步”按钮以及即将开始的模块的标题_
...
* 为不同的培训计划指定量身定制的反馈表。 例如，销售课程可使用以NPS为重点的问题，而合规性课程可使用课程效果和Likert级问题来衡量清晰度和策略理解。
* 根据学习模块使用单独的反馈表单。 例如，混合模块、自学模块和教室模块的三种不同形式。

有关详细信息，请查看[L1反馈表单](/help/migrated/administrators/feature-summary/l1-feedback-form.md)。
...
以前，使用API （`GET /bulkimport/cansync`和`POST /bulkimport/startrun`）迁移带有外部链接的活动模块在访问链接后未向学习者显示&#x200B;**[!UICONTROL 标记为完成]**&#x200B;选项。 此问题已修复。 现在，通过API迁移了外部链接的活动模块将为学习者正确显示&#x200B;**[!UICONTROL 标记为完成]**&#x200B;选项。
...
+++更新 95：2023 年 11 月版 Adobe Learning Manager
...
* 根据学习者接受培训的不同，我们已将字符串“对此培训进行评级”重命名为“对此课程进行评级”、“对此学习路径进行评级”或“对此认证进行评级”。 学习者会看到相应的字符串，具体取决于培训类型。
...
* 处理包含大量记录的 CSV 会花费大量时间，在此期间，如果用户登录并输入其中一个属性的值，则可能会创建一个新用户组，从而可能导致 CSV 错误。 要解决此问题，请在导入CSV的过程中禁用“活动字段”属性弹出消息，并在CSV上传完成后重新启用该消息。
...
* 将 GetNotificationData 从 GET 更改为 POST。 原始实现生成了错误&#x200B;**IllegalArgumentException：请求标头太大**，导致通知失败。
+++
...
本次更新推出全新工作辅助报告，其中可列出帐户中的所有工作辅助。
...
**注意：**&#x200B;在此版本的Adobe Learning Manager中，拥有非活动帐户的用户无法再使用子域访问其帐户。 可以使用帐户 ID 或者转至 acapindex.html 页面并输入电子邮件 ID 来访问帐户。
...
Adobe Learning Manager 的一项[实例](https://learningmanagerapac.adobe.com/acapindex.html)现在将在孟买 (ap-south-1) 的 AWS 上进行托管。 如果客户使用此印度实例，则其用户个人身份信息(PII)和用户学习记录将仅存储在印度地区。
...
* 具有自定义角色权限的用户无法在私密内容文件夹中搜索内容文件。 此问题现已修复。

**课程**

* 无法删除与学习计划有历史关联的课程或学习路径。 此问题现已修复。 现在，用户可以删除当前未与学习计划关联的课程或学习路径。
* 预览课程或学习路径时，如果资源文件名较长且没有空格，则文件名不会按预期换行，会溢出到下一行。 此问题现已修复。
* 之前，在虚拟教室 (VC) 中，无需在其 URL 并不包含必要信息的新实例中选择会议系统，即可创建模块。 现在，在模块创建阶段会显示错误消息，要求指定VC会议系统后才能保存模块，从而有效避免这一情况。
...
* 如果将界面和内容语言设置为德语或西班牙语，管理员无法保存公告的开始和结束日期。 此问题现已修复。
...
* 在欧洲，从 Adobe Learning Manager 直接发布到 Adob&#x200B;&#x200B;e Adobe Learning Manager 的内容默认以德语发布。 此问题现已修复。
...
Adobe Learning Manager 为所有管理员和经理提供合规性信息板。 各经理发现，跟踪其团队成员在特定培训中的合规性非常有用。 同时，管理员希望所有经理将合规性培训添加到他们的信息板并对其进行跟踪。
...
**2021 年 10 月版 Adobe Learning Manager** 引入了学习路径的概念。

>[!NOTE]
>
>**“设置”>“常规”**&#x200B;页面有一个启用学习路径扩展功能的新选项。 如果此选项已启用，您可以在其他学习路径中添加新学习路径。 此选项一经启用，便无法更改。

“学习路径”取代了现有的“学习计划”功能。 设想一下这样的场景：“学习计划”在保全现有功能的情况下获得出色的增强功能。 此外，该功能称为“学习路径”。
...
* 电子邮件模板（比如课程取消注册、学习计划取消注册或认证取消注册）未反映 CSV 中定义的最新产品术语。 现在，电子邮件模板中的默认文本可支持自定义术语。
* “发布至 Adobe Learning Manager”工作流程不支持 Adobe Learning Manager 中的用户语言。 如果用户语言不同，则使用英语“发布至 Adobe Learning Manager”。
* 如果向自定义角色添加的目录过多，则在更新该角色时会发生错误。 现在，目录数量限制已增加至 50。
* 在某些情况下，已删除的培训仍会在目录中显示。 此问题仅发生在管理员应用中，该问题现已修复。
* 当管理员角色从一名用户更改为另一名用户时，UI 中仍会反映前一名用户为管理员角色。 此问题现已修复。 此问题仅针对外部用户，不针对内部用户。
...
* AEM 组件的帮助页重定向至错误的位置。
* 外部化获取和存储用户电子邮件/令牌，以便用户可以实施自己的存储后端，而不是使用 AEM 用户节点。
...
* **富文本格式：** Adobe Learning Manager 现针对课程、计划、证书和工作辅助中的描述提供富文本编辑功能。 该功能允许作者以富文本格式（与纯文本相反）指定描述，包括超链接、图像和其他文本格式选项。
* **星级评分：**&#x200B;学习者现在可以按 5 分制为课程评分。 管理员可以在现有效果评分或 5 星级评分之间进行选择。
* **Badgr 集成：**&#x200B;学习者现在可以授权 Adobe Learning Manager 自动将他们在 Adobe Learning Manager 中获得的徽章推送到 Badgr 帐户，然后他们可以通过 Badgr 帐户在社交网络中分享徽章。
...
* 管理员和管理层查看待审批课程的功能。 管理层可使用该功能确保经理追踪员工学习和培训，并允许 Adobe Learning Manager 管理员根据需要批准课程注册。
...
* 在任何应用程序中访问学习计划时，userGroupInfo API 不可见。
...
CaptivateLearning Manager移动应用程序3.3版本更新引入了全新的主页，支持刊头和基于AI的培训建议。 此主页可用于针对全新“沉浸式布局”选项配置的所有帐户。 使用“经典布局”配置的帐户将继续显示为经典/旧版主页。 这些帐户不会看到主页中的任何更改。

此外，本次更新后，学习者可下载 PDF 和图像格式的徽章。 本次更新还引入了反馈弹出窗口，允许学习者匿名提供对该应用程序的反馈。
...
所有已启用“沉浸式布局”选项的帐户，都有一个支持“沉浸式布局”配置的全新主页。
...
* 为 AICC 内容模块下载的 L2 测验分数报告在“用户总分数/测验分数”这一列下显示的分数不正确。 此问题已修复。
...
* 如果活动字段中包含特殊字符，则 CSV 上传将失败。 此问题已修复。
* 在少数情况下，当学习者参加在 Captivate 中创建的测验时，不会以预期的方式获得答案。
* 创建订阅并尝试编辑订阅后，**保存**&#x200B;和&#x200B;**取消**&#x200B;按钮不会按预期显示。 此问题已修复。

**播放器**

* 对于 SCORM-2004 特定类型的内容，恢复方案不起作用。 因此，学习者必须确定其停止的位置。 此问题现已修复。 现在，可从用户之前停止的位置恢复内容。
* 在某些情况下，注册课程后，不会按预期播放内容。 此问题已修复。

**取消注册**

* 即使有再多的学习者未注册课程/认证，注册报告也只会列出 20 名未注册的学习者。 此问题已修复。
...
* 在导出超过 500 名用户的用户组报告时，报告中的数据值和列标题不匹配，此问题现已修复。
...
* 如果自定义徽章上带有目录标签和空格，则无法按预期方式下载 PDF。
...
* 要确定每个学习者注册的所有课程以及他们是否已完成该课程，请在仪表板的“订阅报告”中包含以下字段：
...
* 在&#x200B;**“添加报告”**&#x200B;对话框中，**“时间跨度”**&#x200B;和&#x200B;**“过滤器”**&#x200B;选择器已作为两个独立部分显示，默认情况下处于折叠状态。 有关更多信息，请参阅&#x200B;[***创建报告***](../administrators/feature-summary/reports.md#report)。

* 在&#x200B;**“添加报告”**&#x200B;对话框中选择用户组时，您可以使用预键入搜索来选择一个或多个用户组。 有关更多信息，请参阅&#x200B;[***用户组报告***](../administrators/feature-summary/reports.md#user-group-reporting)。

**“时间”列值的变更**

在学习者成绩单的时间列中，分钟值将舍入到最近的分钟，秒值则为 00。 有关更多信息，请参阅&#x200B;[***“时间”列***](/help/migrated/administrators/feature-summary/reports/learner-transcripts.md)。

### 此版本中修复的问题 {#Issuesfixedinthisrelease-3}

**学习者信息板**

* 学习日历会显示&#x200B;**“会话已注册”**&#x200B;状态，即使经理尚未批准注册。 现在，在经理批准注册之前，则向学习者显示正确状态&#x200B;**“待处理”**。

* 在特定情况下，对于某个会话，即使学习者已完成课程，学习日历中也会显示&#x200B;**已注册**&#x200B;状态。
...
在轻松随意的环境下分享自己的想法和见解 您可以喜欢或不喜欢某个帖子，也可以上传内容或对帖子进行评价。 有关更多信息，请参阅&#x200B;[***移动应用程序中的社交学习***](../learners/feature-summary/ipad-android-tablet-users.md#socialmobile)。

**在讨论区共享媒体**

将图片、文档、音频或视频文件共享到任意讨论区，以便讨论区其他成员查看您的帖子并开展互动。  有关更多信息，请参阅&#x200B;[***共享帖子***](../learners/feature-summary/ipad-android-tablet-users.md#socialmobile)。
...
将帐户中的技能映射到 Adobe Learning Manager LMS 中的技能域。 这有助于将您的帐户技能与 Adobe Learning Manager 支持的技能域关联，实现自动辅助监管。 有关更多信息，请参阅&#x200B;[***映射技能到域***](../administrators/feature-summary/curation-skills.md)。
...
* 通过共享目录获得学习对象的完整编辑功能。 有关更多信息，请参阅&#x200B;[***共享目录控制***](../administrators/feature-summary/shared-catalog-full-control.md)。 要启用该功能，请联系 Adobe Learning Manager 支持。
* 讲师现在可以轻松识别包含待处理审核的会话和模块。 有关更多信息，请参阅&#x200B;[***待处理审核***](../instructors/feature-summary/learners.md#pending)。

* 技能现在支持授予小数形式的积分值。 这允许作者为特定课程授予小数级积分值。 有关更多信息，请参阅&#x200B;[***小数支持***](../administrators/feature-summary/skills-levels.md#decimal)。
* 自动创建自定义角色。 有关更多信息，请参阅&#x200B;[***通过 csv 文件配置角色***](../integration-admin/feature-summary/configure-role-csv-files.md)。
* 外部认证和活动模块所需的提交现在是可选项。 这有助于经理和讲师可在学习者未提交文件的情况下进行评估。 有关更多信息，请参阅&#x200B;[***可选提交***](../managers/feature-summary/learning-objects.md#optional)。
* 为已删除的用户下载学习者成绩单。 有关更多信息，请参阅&#x200B;[***学习者成绩单***](/help/migrated/administrators/feature-summary/reports/learner-transcripts.md)。
* 支持以下语言：
...
* **扫描二维码进行注册：**&#x200B;二维码功能已得到增强。 除了支持二维码标记出勤情况外，现在还支持使用二维码注册和完成课程。
  ...
  要注册并完成课程，您可以扫描管理员提供的二维码。 <!-- For more information on scanning QR codes in the web version of Learning Manager, see  [***Scan QR code***](https://helpx.adobe.com/captivate-Learning>Manager/whats-new.html#QRcodetoenrollcompleteenrollcompleteacourse) -->.

* **课程多次尝试：** Learning Manager应用程序允许学习者在课程学习中启用多次尝试功能。<!-- For more information on setting up multiple attempts, see  [***Multiple attempts***](<https://helpx.adobe.com/captivate-Learning>Manager/authors/feature-summary/courses.html#Multiattempts). -->
...
* 当将某些用户从某个用户组移除时，这些用户可能无法正确注册加入课程。 此问题现已修复。
...
* 尽管能够在经理应用程序和数据 API 列表中看到认证，但在信息板报告下拉菜单中却看不到该认证。 此问题现已修复。
* 虽然共享目录与 Adobe Learning Manager 帐户无关，由于缺乏权限，管理员仍无法将少数几个学习对象弃用。 此问题现已修复。
...
* 经理所下载的学习者成绩单中可能未显示有关注册/完成等活动的信息。 此问题现已修复。
...
* 此版本 Adobe Learning Manager 推出了诸多精彩的新功能和增强功能。 此版本新增了创建自定义角色、添加目录标签、清除用户、管理标签、重命名学习对象、Slack 集成、新连接器集成、支持 xAPI 等功能。 有关新功能和增强功能的详细信息，请参阅[新功能摘要](../whats-new.md#main-pars_text)。
...
* 当您搜索并选择任何学习者并标记出勤情况时，Adobe Learning Manager 会标记所有学习者的出勤情况。 此问题现已修复。
* Adobe Learning Manager 按 24 小时制在电子邮件中显示时间。 此问题已修复。 现在按 12 小时制显示时间。
...
**问题：**&#x200B;使用 Internet Explorer v11.1478.10586.0 访问少数特定学习对象可能会导致 Adobe Learning Manager 崩溃。

**解决方法：**&#x200B;联系公司 IT 团队，将 Internet Explorer 11 浏览器更新到最新版本。
...
* 如果在默认目录处于禁用状态时创建了新目录，此时如果经理具有这个新建目录的访问权限，则可能无法搜索该目录下的任何课程。 此问题现已修复。
* 移动应用程序用户可能接收不到 L1 反馈的推送通知。 此问题已修复。
...
* 如果作者在源帐户中自动保存了课程，那么对某个共享课程的更改可能不会传输到共享帐户。 此问题现已修复。
* 有时，特定Learning Manager项目中的内容会在流体播放器中冻结。 此问题现已修复。

* “学员信息板”中“学习日历”小组件下的“日历清单”可能随机显示。 此问题现已修复。 现在，日历清单会按顺序依次显示。
* 如果使用“全选”选项标记出勤情况，那么同一节课中各个学习对象的学习者均会标记为“已出勤”。 此问题现已修复。
* 在某些高分辨率屏幕上，Adobe Learning Manager 发出的电子邮件的横幅图像和文本会存在对齐和截断问题。 此问题已修复。
* Adobe Learning Manager 发出的某些电子邮件（不含任何通知数据）不会触发至用户。 例如：关于创建和启用外部个人资料的电子邮件，以及关于配置连接帐户的电子邮件。 此问题已修复。
...
* 在某些情况下，将一个学习计划添加到目录后，添加选项可能仍处于启用状态。 此问题已修复。
* 如果在设备浏览器中打开 Adobe Learning Manager，会显示一个在设备应用程序中体验 Adobe Learning Manager 的选项。 单击“是”之后，如果未安装应用程序，则会启动 Play Store (Android)；如果已安装应用程序，则启动该应用程序 (Android和iOS)。 此项工作流程存在问题，现已修复。
...
* 系统会自动标记他/她的出勤以连接模块。
* 当使用 Adobe Learning Manager 设备应用程序扫描 CR 和 VC 模块的二维码时。
...
* 当单击列表，它可能不会自动折叠并不需要手动关闭视为内容。 此问题现已修复。
...
* 如果用户未下载任何辅助工具，然后单击&quot;我的辅助工具选项卡在脱机模式下崩溃在 Android 可能会在 IOS 的应用程序和显示消息语言中加载数据时出错。 预先解决该问题。
* 若关闭网络之后立即访问课程，即使是已下载的课程，Adobe Learning Manager 应用程序也会结束或显示错误。 此问题现已修复。
* 在某些情况下，当您扫描 QR 码，它显示以前的 QR 码扫描一个捕捉的图像。 此问题现已解决。
* 尝试取消从我的某些场合的辅助工具&quot;选项卡中已添加辅助工具将显示一条错误消息。 此问题已修复。
...
此版本增强了游戏功能的范围。 外部用户现在可以使用游戏功能。 作为管理员，您可以更改范围设置来定义游戏的范围。 您可以选择性地在个人资料相似的用户、群组或位置之间启用游戏。
...
工作辅助是学习者无需注册任何学习对象（例如课程或学习计划）即可访问的培训内容。 管理员可借助此增强功能提取并下载工作辅助报告。 作为管理员，您还可以为所有由您发送的公告生成报告。 管理员和经理还可以检索报告，了解已取消注册的学习者。

**Learning Manager连接器**

现在可以使用“数据导出”选项将用于技能导出到 FTP 位置，从而与任何第三方系统进行集成。 您可以为集成指定“连接名称”并选择是要导入内部用户还是导出用户技能（安排配置或提取）。

**复制课程实例**

您现在可以通过单击实例右下角的下拉箭头来复制实例 URL。
...
在此更新中，Adobe Learning Manager 学习者体验现在支持 Section 508 Standard 辅助功能。 Adobe Learning Manager 也兼容最新版本的 **[!UICONTROL JAWS]**。 此功能仅支持学员应用程序。 使用 Internet Explorer 11、Windows Chrome 或 macOS Safari 来访问此功能。

### 已修复问题 {#IssuesFixed-5}

**部分时区错误信息**

对于部分时区的学习者，剩余天数的截止日期提醒数字不正确。 此问题已修复。

**计划实例过期时的学习计划问题**

如果计划实例过期，则从学习计划启动模块会发生问题。 这样会导出模块扩展无法使用，并且学员无法启动播放器并访问内容。 此问题已修复。
...
现在作者和管理员在创建学习对象时，可为课程、学习方案或认证指定唯一 ID。 如果在创建学习对象时想要启用唯一 ID，请单击“设置”>“常规”。 选中位于“唯一学习对象 ID”选项旁的“启用”复选框。

**学员讨论区**

使用课程中的讨论区与同学和讲师互动。 学员可以查看所有与课程相关的帖子， 然而，您只可以删除自己发布的帖子。 有关讨论区的详细信息，请参阅[查看和参加讨论](../learners/feature-summary/courses.md#main-pars_header_1772461149)。

### 增强功能 {#Enhancements-3}

**X/Y 个课程**

您可以设置课程、认证以及学习方案等学习对象的完成标准，这样学习者只需在模块或课程中完成 X/Y 的内容即可。 类似地，作者也可以对认证以及学习方案的完成标准进行设置。
...
管理员现在能够配置“重置模块”选项，允许学员重置不及格或未完成的课程。

**报告目录**

在 Adobe Learning Manager 中创建报告时，您现在可生成目录报告和图像。
...
管理员现在可以配置课程的具体注册消息并通过电子邮件发送给学员。
...
Adobe Learning Manager 现在可让您添加课程的交付方式。
...
Adobe Learning Manager 现在能够让多位作者同时编辑相同的内容组。

**Adobe Connect 增强功能**

您现在可以使用多个 Adobe Learning Manager 帐户配置单个 Adobe Connect URL。
...
作为管理员，您现可在添加外部用户时添加特定域名。 当学员注册帐户时，只能输入来自这些域名的电子邮件地址。
...
学习者可以使用流体播放器播放按钮两侧的新图标快进或后退 10 秒。 有关这些增强功能的更多信息，请参阅[流体播放器](../learners/feature-summary/fluidic-player.md)。
...
* 在完成课程后，系统会显示 L1 反馈对话框。 学员也会收到有关 L1 反馈完成的通知。
* L1 和 L3 反馈功能中提供了添加描述性问题的选项。 管理员可为学员添加这些描述性问题。 此配置是 Adobe Learning Manager 所提供默认问题集的补充。 您可为 L1 反馈添加两个描述性问题，为 L3 反馈添加一个描述性问题。\
...
* 某些情况下，当管理员尝试使用学员选项卡查看课程注册时，部分已注册的学员姓名不显示。 此问题现已修复。
...
* 在订阅的课程中，学员无法在正常和重新访问模式期间浏览目录内的模块。 此问题现已修复。
...
* 某些情况下，当学员尝试从学习>工作辅助转至学习>课程选项卡时，课程无法载入学习>课程选项卡。 此问题现已修复。
...
* 在学员角色中，即使管理员禁用了学员课程效果，课程效果仍显示为&#x200B;**排序方式**&#x200B;过滤选项之一。 此问题现已修复。
...
* 某些情况下，如果帐户中的非活动或已删除用户超过 20 个，则在学员成绩单对话框的搜索下拉列表中不会显示 20 以后的非活动用户。 此问题现已修复。
* 外部用户帐户过期时，其学员未在生成的学员成绩单中列出。 此问题现已修复。
...
* 在学员成绩单中导出模块数据时发生问题，从而使得到导出的数据不正确。 此问题现已修复。
...
现在，管理员能够自定义课程或学习方案效果功能，从而在学员视图中隐藏效果分数。
...
所有学习对象的所有下载报告和学员列表现在会明确显示被删除的用户。
...
学员访问学员帐户中的工作辅助选项卡时，在加载内容前会显示错误消息“该工作辅助在列表中不存在”。 此问题现已修复。
...
当学习者尝试直接在浏览器中访问目录/学习计划 URL 时，会重新定向至课程。 此问题现已修复。
...
* 有时，为学员工作辅助调用 API 时，显示未经授权访问错误。 此问题现已修复。
...
Adobe Learning Manager 为您提供一项功能，帮助生成您公司 Adobe Learning Manager 学习者的成绩单。 有关更多信息，请参阅[学习者成绩单功能帮助内容](/help/migrated/administrators/feature-summary/reports/learner-transcripts.md)。
...
* 将外部用户组添加到 Adobe Learning Manager 后，您也可以暂停外部用户的注册流程。 另外可随时通过选择“继续”选项撤销暂停。
* 现在可编辑学员的姓名和电子邮件 ID。

**自助注册**

学习者还可取消注册学习对象，如课程、学习方案或认证。 但是学员在完成某个学习对象后，无法取消注册学习对象。

**使用学习对象**

现在，管理员可将学员的学习活动标记为已完成。

**报告**

* 您可订阅课程、学习方案或认证报告。 对于测验分数和学习者状态等数据，还可订阅各课程报告。 订阅将发送至 Adobe Learning Manager 帐户中注册的电子邮件 ID。 您也可以更改此电子邮件 ID。
* 导出认证注册报告时，也将导出名称为&#x200B;**到期日期**&#x200B;的新列。 该列数据能够让管理员了解缺失学习对象使用截止日期的学习者。
...
* 在认证和学习计划的学习者注册列表中，显示学习者的全名（名字和姓氏）。 以前仅显示学员的名字。
...
* 发布标题含有撇号(&#39;)的演示者课程并在Learning Manager中查看后，会显示问号(???) 用于显示在目录中。 此问题现已修复。
...
对于默认的学习计划实例，未显示截止日期。 系统之前肯能会显示预定义的且与用户无关的截止日期。
...
经理姓名、学习对象姓名和 CSV 中用户定义的非唯一数值列，会添加至学习对象的已导出学习者名单中。 以前系统仅显示姓名、日期、电子邮件和状态等学员基本数据。
...
* 学员从&#x200B;**学习**&#x200B;选项卡中删除工作辅助时，**姓名**&#x200B;排序无法按照预期方式工作，直到刷新页面为止。 此问题现已修复。

* 如果工作辅助属于多个课程，则工作辅助列表中未显示课程。 此问题现已修复。
* 如果学员缩放浏览器，工作辅助分页未正确工作。 此问题现已修复。
...
* 外部用户注册页面中添加了文本消息，**用户必须首先注册并创建用户名密码以便日后登录**。

**用户通知**

* 外部学习者单击重新访问课程电子邮件通知中的&#x200B;**打开笔记**&#x200B;链接时，播放器会打开，但笔记面板未运行。 此问题现已修复。
* 外部学员尝试使用重新访问课程电子邮件通知中的&#x200B;**打开笔记**&#x200B;链接打开预习或测试模块时，笔记内容未显示。 此问题现已修复。
...
* 在本地化内容的管理员登录中，将学员注册至课程实例时，操作和注册界面布局失真。 此问题现已修复。
* 作者创建教室或虚拟教室模块时，日历默认月份显示为 2015 年 1 月。 此问题已修复，默认显示当前日期。
* 课程实例名称由斜线和反斜线组成时，导出学员列表失败。 此问题现已修复。
...
外部学习者单击重新访问课程电子邮件通知中的&#x200B;**打开笔记**&#x200B;链接时，发生错误。 此问题现已修复。 该链接会打开播放器及笔记，即使用户未登录 Adobe Learning Manager 也是如此。
...
电子邮件模板中的标题名不正确。 内容已在这些模板中更新，使之可读。
...
管理员可通过更改范围设置来定义游戏范围。 您可以选择性地在个人资料相似的用户、群组或位置之间启用游戏。 有关更多信息，请参阅[游戏](../learners/feature-summary/gamification.md)功能。
...
现在可删除草稿和弃用阶段的课程。 有关更多信息，请参阅[课程](../administrators/feature-summary/courses.md)功能。 如果删除一个学习对象，则会删除所有相关报告数据。 如果删除了某个属于任何其他学习对象的课程，系统会向用户显示相应的消息。

**学习方案和计划**

您可指定学习者在学习方案中学习课程的强制顺序。 您可删除草稿和弃用阶段的学习方案。 如果删除一个学习对象，则会删除所有相关报告数据。

**认证**

可删除草稿和弃用阶段的认证。 如果删除一个学习对象，则会删除所有相关报告数据。
...
学习者报告已强化。 您可在报告中查看姓名、电子邮件、学习对象状态、注册条件、注册日期、完成日期和开始日期。

**添加外部合作伙伴**

为 Adobe Learning Manager 帐户注册外部学习者后，可根据需要缩减学习者数量。 但是无法缩减超过已用名额数的学员。 作为解决方案，您可首先删除已登记的学员并重新为他们注册所需的名额数。

### 已修复问题 {#Issuesfixed-14}

**学员出勤**

管理员视图中的出勤表会显示员工的全名（如果适用）。 系统以前仅显示学员的名字。
...
用户在 Adobe Learning Manager 中单击&#x200B;**[!UICONTROL 注销]**&#x200B;时，现会从 Adobe Learning Manager 应用程序中注销并从 Adobe ID 注销。
...
* 在管理员角色中，学员的全名会在&#x200B;**学员**&#x200B;选项卡中显示。 系统以前仅显示学员的名字。
...
* 以管理员和作者角色访问课程、目录和学习方案时，选项卡的排序更改为&#x200B;**已发布 - 草稿 - 所有 - 已弃用**。 默认选择为&#x200B;**已发布。**
...
* 发送给学员有关学习对象注册和其他活动的电子邮件中，会在电子邮件主题中包含特定的学习对象名称。
...
* 上传超过 500 MB 的 Captivate 课程内容时，内容无法在流体播放器中播放。 该限制被删除。 目前，大小限制已修改为 2 GB。
...
* Adobe Learning Manager 应用程序支持上传 SCORM 内容，即使 manifest.xml 文件中未提及版本也是如此。 版本默认为 1.2。
...
* 学员在注册后收到的欢迎电子邮件中，包含创建 Adobe ID 的 URL 链接。
...
* 在 Android 设备的浏览器上打开学员应用程序 URL 时，会显示弹出窗口表示学员应用程序的可用性。
