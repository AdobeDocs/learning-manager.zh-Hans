...
>[!IMPORTANT]
>
>Adobe Learning Manager 2025年10月版现已上线。 有关最新功能和增强功能的更多信息，请查看[新增功能](/help/migrated/whats-new.md)。 此页面将在下一版本中更新新功能和增强功能。 敬请等待更多更新。
...
有关学习者成绩单报告的更多信息，请参阅此[文章](/help/migrated/administrators/feature-summary/reports/learner-transcripts.md)。
...
管理员可以通过其界面查看由自定义管理员创建的公告。 他们只能通过修改或删除分配的范围来编辑这些公告。 如果未更改范围，管理员将无法进一步编辑公告。
...
6. 在&#x200B;**[!UICONTROL 库]**&#x200B;中，搜索并选择&#x200B;**[!UICONTROL 创建播放列表]**，然后选择以下选项之一：
...
5. 选择播放列表的课程持续时间和类型。
6. 选择&#x200B;**[!UICONTROL 生成播放列表]**。 此时会创建包含10门课程的播放列表，管理员可以使用播放列表创建学习路径。
...
引入了三个新列，以增强在用户、角色和用户角色CSV导出/导入中捕获的数据：

* **用户注册状态(user.csv)**：指示用户的当前注册状态。
* **角色状态(role.csv)**：指示系统中角色的当前状态。
* **用户角色状态(user_role.csv)**：指示用户 — 角色关联的状态。

>[!NOTE]
>
>上述CSV更改仅适用于使用增量用户的帐户。

在此处下载[示例CSV](assets/sample-csv-Incremnetal.zip)。
