...
2. 具有effectiveModifiedDate的&#x200B;**学习对象列表**
...
清单模块中的审阅者注释现在包含在学习者成绩单(LT)中重命名一列下： **审阅者注释**（以前为提交注释）。

#### 影响

学习者和管理员可以在LT导出（UI、作业API和连接器）中查看合并的且标注清晰的审阅者反馈，从而提高透明度和可审核性，并支持更准确的性能评估和指导。
...
| 面积 | 旧列名称 | 新列名称 | 备注 |
| --------------------------- | ------------------ | ------------------ | --------------------------------------------------------- |
| 学习者成绩单（管理员） | 提交注释 | 审核者评论 | 适用于所有管理员许可证来源：UI、作业API、连接器（如果适用）。 |

此更改统一应用于所有管理员LT源（UI导出、作业API报告和基于连接器的导出，如适用）。 连接器导出的LT会将审阅者的注释作为专用列显示在末尾（对于以前未公开提交注释的连接器），从而确保下游集成可以将审阅者反馈与其他注释区分开。

>[!NOTE]
>
>对于学习者的学习者成绩单，之前标记为“提交注释”的列现在重命名为“审阅者的注释”，并在启用后填充清单审阅者的注释。
