...
>
>如果您已将报告自动化设置为每个课程仅占一行，则必须在启用多注册功能之前对报告自动化进行必要的调整。
...
这项更改还将替换 ExaVault 连接器，这将导致新用户无法再使用该连接器。 您可以使用任何开源 FTP 客户端替换 ExaVault。 有关更多信息，请参阅[从 Adobe FTP Manager 过渡](transition-from-ftp-manager.md)。
...
词干提取是将单词简化为词根形式的过程。 这可确保在搜索期间与单词的各种变体匹配。 例如，行走和走路可以简化为相同的根词“走”。 词干设定后，搜索中任一单词的出现次数将与另一个单词的出现次数匹配。

在此版本中，我们为英语语言添加了词干，其中包括以下变体 - en_US、en_AU、en_GB。
...
* 增强课程、学习计划或认证。
* 支持多测验的课程、学习计划或认证。
* Harvard Manage Mentor、内容市场、GetAbstract 或 LinkedIn 课程、学习计划或认证。
* 已启用先决条件的学习计划和证书。
* 已弃用的课程、学习计划或认证。
* 截止日期已过时的课程、学习计划或认证。
* 外部证书。
...
* 在迁移期间无法进行映射。
* 自动标记 LinkedIn、内容市场和其他外部课程、学习计划或认证。
