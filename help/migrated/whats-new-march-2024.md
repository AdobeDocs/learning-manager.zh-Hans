---
description: 了解Adobe Learning Manager 2024年3月版tne的新增功能和增强功能
jcr-language: en_us
title: 新功能摘要
...
>[!NOTE]
>
>观看此[网络研讨会](https://learningmanager.adobe.com/app/learner?accountId=98632#/course/9212121)，了解有关此版本中新增功能的更多信息。
...
如果注册课程的学习者人数很多（例如，超过50,000人），则不会显示学习者列表。 您可以在&#x200B;*搜索学习者*&#x200B;搜索栏中搜索学习者，或者选择搜索栏顶部的&#x200B;**下载**&#x200B;链接以下载学习者列表。

### 管理员 — 学习者页面

搜索任何用户时，**下载学习者**&#x200B;和&#x200B;**导出**选项会下载相同的报告。 同时，在搜索用户组时，您现在可以从该用户组下载筛选后的用户。 搜索用户组时，
**下载学习者列表**&#x200B;更改为&#x200B;**下载用户组的学习者列表****导出**&#x200B;选项会再次下载整个列表。

### 管理员 — 用户页面

#### 内部用户

如果用户数量超过50,000，随后将显示一条消息，下载数据以进行更详细的分析。 搜索栏现在突出显示，并以&#x200B;*姓名、电子邮件的格式显示用户 | UUID*。
...
对于外部用户，同样的行为同样适用。 如果用户数量很大，您可以下载这些用户，还可以在以&#x200B;*姓名，电子邮件格式进行搜索后检索用户的详细信息 | UUID*。

#### “用户清理”页面

在“用户清理”页面上，对于已删除的用户，我们已于&#x200B;**删除日期**&#x200B;删除排序功能。 您只能对UUID进行排序。
...
在Adobe Learning Manager的当前版本中，如果注册认证的用户很多，则您无法查看未注册的学习者，因为&#x200B;**状态**&#x200B;下拉菜单已禁用。

在此版本的Adobe Learning Manager中，如果注册的用户数量较多，则&#x200B;**状态**&#x200B;下拉列表仅显示两个选项：**已注册**&#x200B;和&#x200B;**未注册**。 默认情况下，**已注册**&#x200B;选项处于选中状态。 如果您选择“**取消注册**”，则会显示取消注册的学习者列表。

#### 用户组更改

对于用户组，如果用户组中的用户数少于例如50,000，则&#x200B;**状态**&#x200B;下拉列表会显示所有选项 — “已认证”、“已分配”和“即将过期”。

如果用户组中的用户数很大，则根据新设计，**状态**&#x200B;下拉列表仅显示两个选项 — **已注册**&#x200B;和&#x200B;**未注册**。
...
搜索任何用户时，**下载学习者**&#x200B;和&#x200B;**导出**&#x200B;选项会下载相同的报告。 同时，在搜索用户组时，您现在可以从该用户组下载筛选后的用户。 搜索用户组时，**下载学习者列表**&#x200B;将更改为&#x200B;**下载用户组的学习者列表****导出**&#x200B;选项将再次下载整个列表。
...
根据[管理员设置](administrators/feature-summary/classroom.md#classroom-settings)，作者可以[创建、修改和删除位置](administrators/feature-summary/classroom.md#add-classroom-location)。

>[!NOTE]
>
>添加位置和目录标签时，作者（在课程创建页面中）和管理员（在实例页面中）将分别看到自动填充的位置列表和目录标签。

作为管理员，您可以对作者实施限制以修改或删除教室位置。 查看[教室设置](administrators/feature-summary/classroom.md#classroom-settings)以了解更多信息。
...
创建清单时，作者可以在“强制性清单”部分中选择&#x200B;**启用**。 如果学习者未通过核对表，这样做可防止其继续学习模块。 他们只有通过检查表才能继续。

然后，清单审阅者（如讲师或经理）可以检查清单的状态。 审阅者还可以无序审阅学习者的清单。

### 对清单的重新评估

创建清单时，作者可以在重新评估部分中选择&#x200B;**启用**。 经理或讲师可通过此流程对学习者进行重新评估，直到其通过清单为止。
...
这意味着在创建清单时，“重新评估”部分中的“**启用**”复选框未被选中。 如果选中此复选框，则您可以在“讲师清单”页面上看到“查看/重新评估”按钮。
...
有关详细信息，请查看[创建清单](authors/feature-summary/courses.md#checklist-fail)。
...
在当前版本的Adobe Learning Manager中，当学习者注册Connect VC会话时，会被标记为&#x200B;**未参加**，但不符合完成标准。
...
有关详细信息，请查看[Adobe Learning Manager移动应用程序](white-label.md)中的白色标签。
...
如果您执行迁移以更新现有课程、学习计划或认证，则具有&#x200B;**uniqueLOId**&#x200B;的课程或学习计划或认证会添加到作者应用程序中。

迁移时，您必须更新课程、学习计划或认证的CSV中的&#x200B;**uniqueLOId**&#x200B;值，即使它是可选列。

如果在更新具有&#x200B;**uniqueLOId**&#x200B;的现有课程或学习计划或认证时在执行迁移之前未添加&#x200B;**uniqueLoId**&#x200B;列，则在迁移之后，**uniqueLOId**&#x200B;值将被NULL值覆盖。

>[!NOTE]
>
>**uniqueLoId**&#x200B;列不适用于工作辅助CSV。
...
从[迁移手册](integration-admin/feature-summary/migration-manual.md#csv-specifications-and-sample-csvs)中下载CSV。
...
有关配置连接器的详细信息，请在Adobe Learning Manager](integration-admin/feature-summary/connectors.md)中查看[连接器。

### 登录访问报告更改

Login Access报告仅在过去五个季度可用。 如果任何集成管理员请求按需下载统一导出并选中&#x200B;**登录访问权限**，则Adobe Learning Manager将显示一条错误消息。 但是，对其他报告没有影响。
...
如果您尝试下载超过五个季度的&#x200B;**登录访问**&#x200B;报告，则会显示一条错误消息。 如果您尝试下载的&#x200B;**用户审核**&#x200B;报告超过六个月，则会显示类似的错误消息。

### 已弃用的API

有关产品中所有已弃用API的累积列表，请在Adobe Learning Manager](api-deprecations-list.md)中查看[API弃用。
...
* 在移动应用程序或沉浸模式下，删除或弃用课程实例并选择下一个活动实例后，将显示&#x200B;**注册兴趣**&#x200B;按钮，而不是&#x200B;**注册**。
* 当配对帐户的学习者尝试使用“学习对象API”选择课程缩略图时，会显示错误403“禁止”。

## 系统要求

查看[Adobe Learning Manager系统要求](system-requirements.md)。
